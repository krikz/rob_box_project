...
**Плановые документы — внешняя среда. Читай ровно то, что нужно для текущей фазы.**

```
Только перед стартом:  README.md  (обзор всех фаз)
Перед каждой фазой:   phase-N.md (детали этой фазы только)
По необходимости:     дизайн-документ → только нужная секция
Стандарты:            PEEK → нужная секция → READ
```

❌ Не читай все phase-*.md сразу — они нужны последовательно  
❌ Не загружай весь PYTHON_STYLE_GUIDE — читай только при конкретном вопросе  
✅ THINK в начале каждой фазы: «что именно нужно реализовать?»  

## Перед началом
1. `<план-директория>/README.md` — прочитай **полностью** (он короткий — обзор фаз)
2. Дизайн-документы и стандарты — читай **по мере необходимости** (PEEK → нужная секция)
...
```
1. НАЧАЛО ФАЗЫ
   → THINK: прочитай phase-N.md → что именно нужно реализовать / изменить / создать?
   → Если phase ссылается на дизайн-документ: PEEK → GREP → READ только нужную секцию
   → Сообщи: "Начинаю Phase N: <название> | файлы: <список из phase-N.md>"
