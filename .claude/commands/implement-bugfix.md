...
**План багфикса короткий. Читай ровно то, что нужно для текущей фазы.**

```
Только перед стартом:  README.md  (обзор: 1-2 фазы)
Phase 1 (тест):    THINK → прочитай phase-01.md → GREP существующие тесты
Phase 2 (фикс):     THINK → прочитай phase-02.md → GREP баг в коде → READ только те строки
```

❌ Не читай root-cause и fix-design целиком перед стартом — возьми детали из phase-*.md  
✅ THINK в начале каждой фазы: «что именно нужно сейчас»  

## Перед началом
1. `<план-директория>/README.md` — прочитай **полностью** (он короткий)
2. `root-cause.md` и `fix-design.md` — читай посекционно (PEEK → нужная часть) ходсодержание есть в phase-*.md
