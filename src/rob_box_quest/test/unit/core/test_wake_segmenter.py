...
Модуль ``core/wake_segmenter`` — чистый Python без rclpy: эти тесты
ВЫПОЛНЯЮТСЯ на dev-машине без ROS, а не скипаются (тесты уровня
``QuestBridge`` в ``test/unit/test_quest_bridge*.py`` исторически
пропускаются через ``importorskip``, и однажды это уже привело к ложному
закрытию карточки #1992).
"""
...
def _feed(seg: WakePhraseSegmenter, n_frames: int, start: float = 1000.0):
