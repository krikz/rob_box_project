...
//   * `voice_mode` (на тумблеры STT/LLM/style_off, через
//     ensureLlmFormalize в main.ts:404-465);
//   * `set_voice` (на пресет/язык, через sendStyleChange в main.ts —
//     этот cmd менял voice_preset на dialogue_node, ЛИЧНОСТЬ, не grip pipeline).
//
// ADR-0087 (2026-09-09, вариант (a)): legacy `set_voice` style/language
// ветка удалена из ws_server вместе с подписками супервизора на
// `/avatar/set_voice_preset|language` (канал был «честный no-op» после
// PR #2255, без владельца на `/dialogue/control`). Остался единственный
// путь смены стиля/языка грипа с панели — `voice_pipeline` cmd ниже.
//
...
//
// Команда `voice_pipeline` (см. ws_server.py) валидирует whitelist
