...
class AnimationPlayerNode(Node):
    ...
    def __init__(self):
        ...
        self.load_subscription = self.create_subscription(String, "~/load_animation", self.load_animation_callback, 10)

        # Subscription for voice animation requests (from GUI or dialogue)
        self.voice_animation_subscription = self.create_subscription(
            String, "/voice/animation/request", self.voice_animation_callback, 10
        )

        # Subscription for TTS state (auto-switch animations)
        self.tts_state_subscription = self.create_subscription(String, "/voice/tts/state", self.tts_state_callback, 10)
        ...
        self.get_logger().info("✅ Subscribed to /voice/tts/state for automatic animation switching")
        self.get_logger().info(f"   Idle animation: {self.idle_animation}")
        self.get_logger().info(f"   Talking animation: {self.talking_animation}")

        # Services
        self.srv_play = self.create_service(Trigger, "~/play", self.play_callback)

        self.srv_stop = self.create_service(Trigger, "~/stop", self.stop_callback)

        self.srv_pause = self.create_service(SetBool, "~/pause", self.pause_callback)

        self.srv_list = self.create_service(Trigger, "~/list_animations", self.list_animations_callback)
    ...
    def tts_state_callback(self, msg):
        ...
        if state in ["synthesizing", "playing"]:
            ...
            # ВСЕГДА переключаемся на talking, даже если активна ручная анимация
            if not self.is_robot_speaking:
            ...
            # Robot stopped speaking - switch back to idle or emotion animation
            if self.is_robot_speaking:
                self.get_logger().info("🤐 Робот замолчал - возвращаюсь к предыдущей анимации")
                self.is_robot_speaking = False

                # Если ручная анимация ещё активна (таймер не истёк) - возвращаемся к idle
                # Таймер сам переключит на idle когда истечёт
                if self.manual_animation_active:
                    self.get_logger().info("⏪ Ручная анимация всё ещё активна, возврат к idle")
                    self.manual_animation_active = False  # Сбрасываем флаг
