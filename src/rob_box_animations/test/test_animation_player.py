...
class TestAnimationPlayerPlayAnimation:
    ...
    def test_play_animation_cancels_previous_playback(
        ...
        ``is_playing`` guard and returned False while the old thread
        kept running — exactly the production race we are closing.
        """
    ...
    def test_play_animation_does_not_warn_already_playing(
        ...
        Production log evidence:
            voice_v3_postdeploy.log:278,318,...,520 — 21 occurrences
            of 'Animation already playing' in 145s of dialog.
        """
    ...
    def test_play_animation_skip_reload_when_same_manifest(
        ...
        """Reload-skip path: same manifest requested twice = 1 disk load."""
        with patch.object(
            player.loader,
            "load_manifest",
    ...
    def test_play_animation_thread_safety_under_concurrent_calls(
        ...
        Reproduces the production race: many parallel requests coming
        in (TTS state + voice requests + idle-fallback timer firing
        at once). Before the fix this left multiple playback threads
        alive; after the fix exactly one ``playback_thread`` is
        tracked.
        """
