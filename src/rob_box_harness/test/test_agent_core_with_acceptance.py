"""Integration tests for ``AgentCore`` + :class:`AcceptanceGate`.
...
helpers from :mod:`rob_box_harness.test.test_agent_core` — but
to keep this file self-contained we redefine a minimal subset
rather than depending on the agent_core test module's private
...
# ---------------------------------------------------------------------------
# Minimal fakes — duplicated from test_agent_core to keep this file
...
class _FakeLLM:
    ...
    def __init__(self, *, tool_calls: list[ToolCall] | None = None, final_text: str = "ok") -> None:
        ...
        # Every complete() call's message list — lets a test assert what
        # the LLM actually saw on its second turn (e.g. the acceptance
...
async def _drive_to_dialogue(core: AgentCore) -> None:
...
def test_require_tool_call_surfaces_sentinel_result_to_llm() -> None:
    ...
    assert llm.complete_calls == 2
    msgs = llm.calls[-1]
    # Verify the sentinel made it into the tool message of the second turn.
