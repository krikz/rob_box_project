...
FACE_RECOGNITION_PARAMS = {
    ...
    # ЗАГЛУШКА до настоящего sweep по ADR-0123 §6 (issue #2771), НЕ
    # калиброванные значения — как голосовой в #2348, а не «на глаз».
    # Прежний дефолт 0.45 пускал чужих: тёща владельца опознана как
    # «Деньчик» при score=0.483 (issue #2771). identify — «похож
    # достаточно, чтобы назвать имя»; enroll — заметно строже, «похож
    # достаточно, чтобы дописать эмбеддинг в галерею» (issue #2772,
    ...
    # min_face_px проверяют только геометрию детекции — RetinaFace может
    # полчаса честно детектировать тень в тёмной комнате, бокс большой,
    # а кроп почти чёрный. ArcFace на таком входе не падает, а стабильно
    # отдаёт один и тот же вырожденный эмбеддинг, который ложится в
    # галерею как "человек" с аномально однородной (0.90+) близостью
    # векторов. Числа — живой замер на Vision Pi 22.09.2026 (issue #2749):
    # фантомная запись 8ffc2641-... (130 "встреч" тени) — средняя
    # яркость 6.4-7.4/255, контраст(p95-p5) 28-31; живой человек в той же
    # базе (b49470e1-...) — яркость 76.6-185.4/255, контраст 170-208.
    # Подробное обоснование чисел — DEFAULT_MIN_CROP_MEAN/
    # DEFAULT_MIN_CROP_CONTRAST в face_recognition.py.
    'min_crop_mean': '20.0',
    'min_crop_contrast': '25.0',
    # После прогрева ArcFace ~8-25 мс на лицо (замер 22.09.2026),
    # так что это не бюджет NPU, а защита от хвоста мелких лиц.
