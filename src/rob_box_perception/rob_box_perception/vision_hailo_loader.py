...
def _is_nms_postprocessed_output(raw_output: Any) -> bool:
    ...
    ПРИМЕЧАНИЕ (честность, ADR-0018): код ИНТЕРПРЕТИРУЕТ каждую строку
    как ``[y_min, x_min, y_max, x_max, score]`` (столбцы 0..4 в этом
    порядке) — это **свойство реализации**, подтверждённое синтетическим
    тестом ``test_post_process_ragged_nms_output_yolov8n_person``
    (``src/rob_box_perception/test/unit/test_vision_hailo_phase15.py``):
    тест передаёт фиктивный per-class ndarray
    ``[[0.2, 0.3, 0.8, 0.7, 0.87]]`` и проверяет, что выход проходит
    пороги и попадает в events (т.е. confidence трактуется как
    ``row[4]``). Сам по себе тест не различает y-first и x-first
    layout (порядок y/x/y/x в ragged-выводе из HEF меняется местами
    только если менять оси симметрично — обе трактовки дают одинаковый
    bbox для симметричных входов). ЭМПИРИЧЕСКАЯ проверка, что HailoRT
    РЕАЛЬНО выдаёт y-first layout (а не x-first вариант), остаётся
    отдельной задачей **issue #2717 acceptance #3** — требует живого
    AI HAT+ (Vision Pi 10.1.1.21) + человека в кадре OAK-D + движения
    влево/вправо (асимметричный bbox). До подтверждения layout
    предполагается правильным (best-documented).
