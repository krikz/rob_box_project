...
class DialogueNode(Node):
    ...
    def _make_tools(self) -> list:
        ...
        async def register_speaker(name: str) -> str:
            ...
            ОБЯЗАТЕЛЬНО вызывай в следующих случаях:
            - "запомни мой голос как X" / "запомни меня как X"
            - "меня зовут X, запомни" / "я X, запомни меня"
            - "запомни кто я" / "это X, запомни голос"
            - "зарегистрируй меня как X"
            БЕЗ вызова этого инструмента имя сохраняется только в текстовой памяти —
            голосовое распознавание работать НЕ будет! После регистрации подтверди голосом."""
            msg = String()
