    ...
    def _build_dj_prompt(self, n: int) -> str:
        ...
        if n == 1:
            ...
            return (
                ...
                "0) Если в теме вечеринки упомянут артист/группа — вызови search_artist_style(artist_name, song_names) "
                "   чтобы узнать жанр, стиль, BPM, тональность, инструменты. "
                "   Результат используй для подбора треков и стиля музыки. "
                "0.1) Вызови list_tracks() с фильтром tag= для ключевых слов из темы вечеринки. "
