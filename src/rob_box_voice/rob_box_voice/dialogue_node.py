...
class DialogueNode(Node):
    ...
    def _make_output_tools(self) -> list:
        ...
        return [speak_text, play_sound, play_animation, save_dj_set_plan]
    ...
    def _on_stt(self, msg: String) -> None:
        ...
        if self._dj_mode_enabled and self._dj_set_plan:
            dj_ctx = (
                f"[🎧 DJ-РЕЖИМ АКТИВЕН, переход #{self._dj_transition_count}. "
                f'Тема: "{self._dj_theme}". '
                f"Текущий план сета:\n{self._dj_set_plan}\n"
                "Если пользователь просит изменить музыку или план — вызови save_dj_set_plan() с обновлённым планом "
    ...
    def _build_dj_prompt(self, n: int) -> str:
        ...
        if n == 1:
            ...
            return (
                ...
                "Представься голосом как DJ ROB-BOX и объяви тему. "
