...
class DialogueNode(Node):
    ...
    def _build_dj_prompt(self, n: int) -> str:
        ...
        return (
            ...
            f"{theme_block}"
            "Сделай DJ-переход: запусти новый трек/грув через execute_music_code. "
            "⚠️ ОБЯЗАТЕЛЬНО: первая строка кода = Clock.clear() — это очищает старые паттерны и предотвращает какофонию! "
            "Затем определи 2-3 ударных (d1-d3) и 2-3 мелодических (p1-p3) паттерна — максимум 6 слоёв. "
            "BPM минимум 60 (для энергичного: 90-145, для амбиента: 60-90). "
            "sus не более 8. Не используй p_all.stop() / d_all.stop() — они не существуют! "
            ...
            "Главное — чистый звук без перегруза и хорошее настроение публики."
        )
