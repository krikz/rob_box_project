    ...
    def _build_dj_prompt(self, n: int) -> str:
        ...
        if self._dj_theme:
            theme_block = (
                ...
                "Музыка и MC-фразы должны отражать тему. "
                "Изредка (каждые 2-3 перехода, не каждый раз!) — тематическая MC-фраза: "
                "поздравление, отсылка к теме, шутка в духе DJ ROB-BOX. "
            )
        else:
            theme_block = (
        ...
        return (
            ...
            f"{phase_hint} "
            f"{theme_block}"
            "Сделай DJ-переход через execute_music_code. "
