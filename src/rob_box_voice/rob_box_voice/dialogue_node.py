...
class DialogueNode(Node):
    ...
    def _make_tools(self) -> list:
        ...
        async def register_speaker(name: str) -> str:
            ...
            - "меня зовут X" / "я X" / "это X" / "зови меня X"
            - "запомни мой голос как X" / "зарегистрируй меня как X"
            СЛОВО "ЗАПОМНИ" НЕ ОБЯЗАТЕЛЬНО — любое представление = вызов этого инструмента.
            БЕЗ вызова этого инструмента система НЕ распознаёт голос — имя хранится
            только в тексте и другие люди тоже получат ответ на вопросы об имени!
            После вызова подтверди голосом: 'Запомнил! Теперь буду узнавать тебя по голосу, [имя].'"""
            msg = String()
