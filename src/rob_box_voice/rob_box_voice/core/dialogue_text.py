...
DEFAULT_WAKE_WORDS: tuple[str, ...] = (
    ...
    "бот",
    "роб",
    # wake-имя личности «ТАРС» (issue #2097): персонаж «Пикника на обочине» —
    # теперь принимается и в ReSpeaker-канале как wake-word личности.
    # Одновременно остаётся в operator namespace (шлем) для маршрутизации
    # к агенту оператора. STT-искажения для «ТАРС» наполняем по логам e2e
    # (целевая §14.1) в operator namespace; в personality держим только
...
# из ReSpeaker-канала wake-word «ТАРС» теперь ТАКЖЕ срабатывает, но в
# personality namespace (issue #2097) — это wake для личности, не для
# агента оператора. Маршрутизация в /avatar/stt/result по-прежнему только
# из wake-потока шлема.
