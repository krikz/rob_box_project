...
class DJModeController:
    ...
    def build_auto_prompt(self, n: int) -> str:
        ...
        library_line = (
            ...
            "готовый человеческий трек; 2) load_track(name=...) — используй "
            "его код с настоящей драматургией; 3) search_samples(<стиль>) — "
            "найди реальные сэмплы. НЕ упрощай до статичного лупа."
        )
        if n == 1:
            return (
