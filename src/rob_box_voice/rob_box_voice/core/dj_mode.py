...
class DJModeController:
    ...
    def is_music_only_transition(self, n: int) -> bool:
        ...
        Говорит модель ТОЛЬКО на переходе #1 («СТАРТ ВЕЧЕРИНКИ» — представление
        диджея). Всё остальное, включая финальный трек плана, — только музыка:
        прощание произносит хук ``_on_dj_stop_farewell`` при выключении DJ,
        а не модель. Иначе финальный трек давал два прощания подряд
        (speak_text модели + фраза хука).
