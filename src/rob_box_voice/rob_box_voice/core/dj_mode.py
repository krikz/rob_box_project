...
class DJModeController:
    ...
    def _apply_enable_payload(self, data: dict, *, is_fresh_start: bool) -> None:
        ...
        #
        # 🔴 FIX (issue #992): модель почти на КАЖДОМ переходе присылает
        # ``set_dj_mode(plan=...)`` с планом, переписанным своими словами
        # (промпт этого не запрещает) — строка плана не является надёжным
        # сигналом «новый сет». Если сбрасывать ``transition_count`` при
        # любом расхождении строки, то ``build_auto_prompt(1)`` снова
        # выдаёт «[DJ_AUTO — СТАРТ ВЕЧЕРИНКИ]», модель снова сочиняет план
        # и снова вызывает ``set_dj_mode(plan=...)`` — бесконечный цикл,
        # сет НИКОГДА не доходит ни до финального трека, ни до auto-stop
        # (оба гейтятся на ``transition_count`` vs длину плана в ``tick``).
        # Живой прогон 01.09 («вечеринка у черепашек-ниндзя»): 8 rewrite
        # подряд за 12 минут, ни одного перехода дальше #1.
        #
        # Контракт (``test_dramaturgy_fix_1016``):
        #   * генуинный старт (DJ был выключен, ``is_fresh_start=True``) —
        #     счётчик с нуля, иначе наследие предыдущей сессии
        #     заставит ``build_auto_prompt(1)`` притвориться «СТАРТ
        #     ВЕЧЕРИНКИ» уже не для новой партии;
        #   * первый план внутри идущей сессии (``is_fresh_start=False``,
        #     ``set_plan`` был пустой) — НЕ трогаем счётчик, иначе
        #     следующий tick снова запустит «СТАРТ ВЕЧЕРИНКИ»;
        #   * переписывание плана в идущей сессии
        #     (``is_fresh_start=False``, ``set_plan`` уже непустой) —
        #     счётчик НЕ трогаем: переписанный текст ≠ новый сет,
        #     прогресс должен сохраниться (иначе см. живой прогон 01.09).
        plan = data.get("plan")
        if plan and isinstance(plan, str) and plan.strip():
            ...
            if new_plan != self.state.set_plan:
                ...
                if is_fresh_start:
                    ...
                    suffix = " (fresh start, counter reset to 0)"
                else:
                    suffix = (
                        f" (updated mid-session, progress kept at "
