...
UNKNOWN_MELODY_CLAIM_PATTERNS: tuple = (
    r"не\s+зна\w+\s+так\w+\s+мелоди",        # не знаю такой мелодии
    r"не\s+зна\w+\s+эт\w+\s+мелоди",          # не знаю этой мелодии
    r"не\s+зна\w+\s+(?:эт\w+|так\w+)\s+произведен",  # не знаю этого произведения
    r"не\s+зна\w+\s+(?:эт\w+|так\w+)\s+трек",  # не знаю этого трека
    r"не\s+зна\w+\s+(?:эт\w+|так\w+)\s+песн",  # не знаю этой песни
...
def detect_unknown_melody_claim(
    *,
    ...
    # («известная мелодия / классика»). Проверяем по двум источникам:
    # (a) BABBLE_PERFORMANCE_KEYWORDS — общий «сыграй/играй/музык/мелоди/трек»
    #     (избегаем глагол «пой» — там Bug C закрывает другой класс);
    # (b) явные маркеры «по имени / классика / известная».
    low_user = user_input.lower()
    user_keyword_hit = any(kw in low_user for kw in (
        "сыграй", "играй", "мелоди", "трек", "композиц",
    )) and any(kw in low_user for kw in (
        "сыграй", "играй", "мелоди", "трек", "композиц", "песн",
    ))
