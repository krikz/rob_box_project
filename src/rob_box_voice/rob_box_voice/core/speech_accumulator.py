...
_INSTRUCTION = (
    "Ниже — фразы, которые пользователь произнёс без обращённого wake-слова "
    "до этого обращения. Это запрос пользователя, который плохо распознался: "
    "выполни его. НЕ приветствуй и НЕ переспрашивай. Если текущая фраза — "
    "только wake-слово или «повтори», ответь на последний явный запрос из "
    "этого списка, не пересказывая фразы дословно."
)
