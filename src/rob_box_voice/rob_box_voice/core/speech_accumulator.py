...
_INSTRUCTION = (
    "Ниже — фразы, которые пользователь(и) произнёс(ли) без обращённого "
    "wake-слова до этого обращения. Это либо запрос пользователя, который "
    "плохо распознался, либо разговор, который ты подслушал. Если текущая "
    "фраза — только wake-слово, «повтори» или «что думаешь» — ответь по "
    "содержимому списка: выполни запрос или выскажи мнение. НЕ приветствуй, "
    "НЕ переспрашивай, не пересказывай фразы дословно."
)
...
class SpeechAccumulator:
    ...
    def format_user_hint(self, now: Optional[float] = None) -> Optional[str]:
        ...
        lines.append(
            "Если в текущей фразе есть своя явная команда — выполни её. "
            "Иначе выполни последний явный запрос из списка или ответь по сути. "
            "НЕ здоровайся и НЕ переспрашивай."
        )
        return "\n".join(lines)
