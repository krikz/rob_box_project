...
def render_event_instructions(
    ...
    *,
    ...
    if faq_store_available:
        ...
        lines.append(
            "Если после FAQ нужен бит или музыка, сначала получи факты, потом при необходимости вызови "
            "execute_music_code / load_track, и только после этого озвучивай ответ."
        )
    lines.append(
...
def build_event_faq_prefetch_context(
    *,
    ...
    lines = [
        ...
        "Если нужен бит или музыкальный фон, сначала используй факты ниже, затем при необходимости вызови execute_music_code / load_track, а потом озвучь ответ.",
