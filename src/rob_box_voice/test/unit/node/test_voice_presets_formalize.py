...
@requires_formalizer
class TestFormalizeFallbackOnLlmFailure:
    """Если LLM вернул пустоту / упал по таймауту / provider error —
    робот НЕ молчит, а проговаривает фразу дословно через ``_speak_direct``
    (как в ``quest_ttts``). Иначе оператор останется без ответа."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            "",                # пустая строка
            "   ",             # whitespace
            None,              # None (для провайдеров, которые вернут None при ошибке)
        ],
    )
    def test_empty_or_whitespace_rewrite_falls_back_to_direct(
        self, monkeypatch: pytest.MonkeyPatch, llm_return
    ):
        """LLM вернул пустоту → fallback на _speak_direct с ОРИГИНАЛЬНОЙ фразой."""
        _install_fake_rcl_interfaces(monkeypatch)
        n = self._setup_node_with_preset()
        ...
        scheduled = MagicMock()
        scheduled.result.return_value = llm_return
        n._loop.run_coroutine_threadsafe = MagicMock(return_value=scheduled)

        from std_msgs.msg import String  # type: ignore[import-not-found]
        msg = MagicMock(spec=String)
        original_text = "Оригинальная фраза оператора"
        msg.data = original_text

        n._on_quest_stt(msg)

        # Fallback должен сработать
        assert n._speak_direct.called or n._tts_publisher.publish.called, (
            f"_on_quest_stt: при пустом ответе LLM ({llm_return!r}) НИ _speak_direct, "
            f"НИ _tts_publisher НЕ вызваны — оператор останется без ответа!"
        )

    def test_timeout_falls_back_to_direct(self, monkeypatch: pytest.MonkeyPatch):
        """LLM завис по таймауту → fallback на _speak_direct."""
        _install_fake_rcl_interfaces(monkeypatch)
        n = self._setup_node_with_preset()

        async def _fake_formalize(self, text, **kwargs):
            raise TimeoutError("LLM timeout ({}s)".format(kwargs.get("timeout")))
        monkeypatch.setattr(
            type(n), "_formalize_with_llm",
            AsyncMock(side_effect=lambda text, **kw: _fake_formalize(n, text, **kw)),
        )

        # run_coroutine_threadsafe.result() бросает TimeoutError
        scheduled = MagicMock()
        scheduled.result.side_effect = TimeoutError("LLM timeout 5.0s")
        n._loop.run_coroutine_threadsafe = MagicMock(return_value=scheduled)

        from std_msgs.msg import String  # type: ignore[import-not-found]
        msg = MagicMock(spec=String)
        msg.data = "Какая-то фраза"

        n._on_quest_stt(msg)

        assert n._speak_direct.called or n._tts_publisher.publish.called, (
            "_on_quest_stt: при таймауте LLM fallback НЕ сработал — "
            "робот замолчит после формализатора"
        )

    def test_provider_error_falls_back_to_direct(
        self, monkeypatch: pytest.MonkeyPatch
    ):
        """Провайдер LLM бросил исключение → fallback на _speak_direct."""
        _install_fake_rcl_interfaces(monkeypatch)
        n = self._setup_node_with_preset()

        async def _fake_formalize(self, text, **kwargs):
            raise RuntimeError("Provider 503: rate limit")
        monkeypatch.setattr(
            type(n), "_formalize_with_llm",
            AsyncMock(side_effect=lambda text, **kw: _fake_formalize(n, text, **kw)),
        )
        ...
        scheduled = MagicMock()
        scheduled.result.side_effect = RuntimeError("Provider 503: rate limit")
        n._loop.run_coroutine_threadsafe = MagicMock(return_value=scheduled)
        ...
        assert n._speak_direct.called or n._tts_publisher.publish.called, (
            "_on_quest_stt: при ошибке провайдер LLM fallback НЕ сработал"
        )


# ─────────────────────────────────────────────────────────────────────────────
#  Sanity-проверка: skip-маркер работает (тесты (3,4) skip-ятся до merge)
# ─────────────────────────────────────────────────────────────────────────────
