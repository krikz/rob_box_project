...
``ROB_BOX_PACK1_LOOPS=1``; по умолчанию выключен. Пак 0 (``foxdot``)
доступен всегда — на него запрет никогда не распространялся.
...
def loop_denial(arg: str, enabled: bool) -> Optional[str]:
    ...
    if info.pack == 1 and not enabled:
        return (
            f"Луп {info.name!r} из пака 1_pitchglitch_samples ещё не "
            "прослушан на роботе (16 kHz DAC) — белый список выключен "
            f"(флаг {PACK1_LOOPS_ENV}). Играй без лупа или loop('foxdot')."
        )
    return None
