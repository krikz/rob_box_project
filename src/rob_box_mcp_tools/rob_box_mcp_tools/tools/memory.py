...
class MemorySaveTool(MCPTool):
    ...
    def description(self) -> str:
        return (
            "Сохранить факт/предпочтение пользователя в долгосрочную память "
            "(имя, привычки, настройки). Сохраняется между сессиями."
        )
    ...
    def parameters(self) -> List[MCPToolParameter]:
        return [
            MCPToolParameter(
                name="fact",
                type="string",
                description="Текст факта, например: 'Пользователя зовут Алексей'",
                required=True,
            ),
            MCPToolParameter(
                name="category",
                type="string",
                description="Категория: preference, habit, name, general",
                required=False,
...
class MemorySearchTool(MCPTool):
    ...
    def description(self) -> str:
        return (
            "Поиск по долгосрочной памяти (все сессии). Используй чтобы вспомнить: "
            "'где мы остановились', 'что я просил раньше'."
        )
...
class MemoryContextTool(MCPTool):
    ...
    def description(self) -> str:
        return (
            "Получить контекст из предыдущих сессий: последние реплики + известные факты. "
    ...
    def parameters(self) -> List[MCPToolParameter]:
        return [
            MCPToolParameter(
                name="limit",
                type="integer",
                description="Сколько последних реплик загрузить (по умолчанию 10)",
                required=False,
            ),
            MCPToolParameter(
                name="query",
                type="string",
                description="Поисковый запрос — если задан, вернёт релевантные реплики",
                required=False,
            ),
        ]
...
class FaqSearchTool(MCPTool):
    ...
    def description(self) -> str:
        return (
            "Поиск по FAQ активного мероприятия (программа, локация, организация). "
            "Возвращает вопрос-ответные пары из загруженного FAQ."
        )
    ...
    def parameters(self) -> List[MCPToolParameter]:
        return [
            MCPToolParameter(
                name="query",
                type="string",
                description="Поисковый запрос о мероприятии",
                required=True,
            ),
            MCPToolParameter(
                name="limit",
                type="integer",
                description="Максимум результатов (по умолчанию 3, максимум 10)",
                required=False,
            ),
        ]
