...
class StartMappingTool(MCPTool):
    ...
    def description(self) -> str:
        return (
            ...
            "Вызывай ТОЛЬКО при явной просьбе СКАНИРОВАТЬ/КАРТИРОВАТЬ/СТРОИТЬ КАРТУ "
            "('начни картографирование', 'построй карту'). НЕ вызывай для ролевых игр, "
            "экскурсий или разговоров о местах. "
            "map_name → новая карта (new_location=true); 'продолжить' → new_location=false."
        )
...
class ContinueMappingTool(MCPTool):
    ...
    def description(self) -> str:
        return (
            "Продолжить картографирование (SLAM) для добавления новых областей к карте."
        )
...
class FinishMappingTool(MCPTool):
    ...
    def description(self) -> str:
        return (
            "Завершить картографирование и перейти в режим навигации по готовой карте. "
            "Можно указать имя карты."
        )
...
class OptimizeMapTool(MCPTool):
    ...
    def description(self) -> str:
        return (
            "Оптимизировать карту после картографирования: loop closures, "
            "bundle adjustment, очистка occupancy grid, backup."
        )
...
class LoadMapTool(MCPTool):
    ...
    def description(self) -> str:
        return (
            "Загрузить сохранённую карту и перейти в режим навигации (локализации)."
        )
