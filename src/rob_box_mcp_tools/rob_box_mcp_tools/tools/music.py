...
# Schema only — bootstrap track is inserted via _bootstrap_track() using
# parameterised Python INSERT OR IGNORE to avoid SQL string escaping issues.
_INLINE_DDL = """
...
class TrackLibrary:
    ...
    def _apply_migration(self) -> None:
        ...
        self._bootstrap_track()

    def _bootstrap_track(self) -> None:
        """Вставить первый авторский трек робота (INSERT OR IGNORE).

        Используется когда migration-файл недоступен (Docker без volumes/migrations).
        Параметризованный INSERT — не требует экранирования кавычек в коде трека.
        """
        ...
        with self._lock:
            self._conn.execute(
                ...
                (
                    ...
                    json.dumps(["full_track", "minor", "132bpm", "C#", "atmospheric", "robot_authored"],
                               ensure_ascii=False),
                    5,
