#!/usr/bin/env python3
"""
deepseek_adapter.py - Адаптер для интеграции MCP tools с DeepSeek LLM

Этот модуль обрабатывает tool_calls из DeepSeek API и вызывает соответствующие
MCP инструменты через ROS 2 топики.
"""
...
class DeepSeekToolCallAdapter:
    """
    Адаптер для обработки tool calls из DeepSeek API
    ...
    def process_tool_calls_from_message(self, message: Any) -> List[Dict[str, Any]]:
        """
        Обработать tool_calls из ответа DeepSeek API
