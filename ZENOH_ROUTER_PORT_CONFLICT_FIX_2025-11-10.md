...
├─────────────────────────────────────────────────────────────────┤
│  Облачный Zenoh router для удалённого мониторинга                │
│  listen: [::]:7447 (публичный доступ)                            │
└─────────────────────────────────────────────────────────────────┘
                              ↑
                              │ connect (через интернет)
                              │
┌─────────────────────────────────────────────────────────────────┐
│                    Main Pi (10.1.1.10 eth0)                      │
├─────────────────────────────────────────────────────────────────┤
│  zenoh-router (HUB для межмашинной связи + облако)              │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │ connect:                                                    │ │
...
**Ключевые моменты:**
- **Cloud → Main Pi:** Двунаправленное соединение (Main Pi подключается как клиент)
- **Main Pi → Vision Pi:** Двунаправленное соединение (Vision Pi подключается как клиент)  
- **Vision Pi ↔ Main Pi ↔ Cloud:** Данные передаются через установленные соединения
- **Main Pi** является HUB'ом - связывает Vision Pi с облаком
