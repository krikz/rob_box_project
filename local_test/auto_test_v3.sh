...
# Источник: анализ rtabmap.launch.py + demo_hector_mapping.launch
...
echo "===================================================================================="
echo "TEST $TEST_NUM: depth:=false + subscribe_scan:=true"
echo "КЛЮЧЕВОЕ РЕШЕНИЕ: depth:=false отключает subscribe_rgb и subscribe_depth!"
echo "===================================================================================="
...
echo "--- 1. Параметры subscribe_* (ОЖИДАЕМ: scan=true, rgb=false, depth=false) ---"
grep -E "subscribe_scan|subscribe_rgb|subscribe_depth|subscribe_rgbd" "$RTABMAP_LOG" | head -15

echo ""
echo "--- 2. Подписки rtabmap (ОЖИДАЕМ: только /scan, БЕЗ camera топиков!) ---"
