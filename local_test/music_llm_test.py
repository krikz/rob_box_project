...
SYSTEM_PROMPT = textwrap.dedent(f"""
    ...
    ✅ Правильный минимальный Имперский марш (главная тема A+B, BPM=102 из MIDI):
       Clock.bpm = 102
       p1 >> brass(midinote=[67, 67, 67, 63, 70, 67, 63, 70, 67,   # фраза A
                              74, 74, 74, 75, 70, 66, 63, 70, 67],  # фраза B
                   dur=[1, 1, 1, 0.75, 0.25, 1, 0.75, 0.25, 2,
                        1, 1, 1, 0.75, 0.25, 1, 0.75, 0.25, 2],
                   amp=0.9, sus=0.45, room=0.2)  # sus: короткие 0.25-ноты чуть легато, длинные 1-2 — чёткие
    ...
    ❌❌❌ ЗАПРЕЩЕНО для Имперского марша — прочти ДО написания кода:
    ...
    ✅ ТОЛЬКО: p1(brass мелодия) + d1(кик) + d2(снейр) = РОВНО 3 слоя, больше НЕ НУЖНО!
    ✅ Правильный код (A+B = тема, РОВНО 3 слоя — НЕ добавляй больше!):
       Clock.bpm = 102  # точный BPM из MIDI файла
       Root.default = "G"
       Scale.default = "minor"
       p1 >> brass(
           midinote=[67, 67, 67, 63, 70, 67, 63, 70, 67,   # фраза A: G G G Eb Bb G Eb Bb G
                     74, 74, 74, 75, 70, 66, 63, 70, 67],  # фраза B: D D D Eb Bb F# Eb Bb G
           dur=[1, 1, 1, 0.75, 0.25, 1, 0.75, 0.25, 2,
                1, 1, 1, 0.75, 0.25, 1, 0.75, 0.25, 2],
           amp=0.9, sus=0.45, room=0.2  # sus: короткие 0.25-ноты чуть легато, длинные 1-2 — чёткие
       )
       ...
       # ← СТОП! Здесь код заканчивается. НЕ добавляй p2, p3, p4!
    ...
    ❌❌❌ СТРОГО ЗАПРЕЩЕНО — прочти ДО написания кода:
       ❌ p3 >> arpy(...) — это ЧЕТВЁРТЫЙ СЛОЙ, он ЗАПРЕЩЁН! Даже с amp=0.15!
          degree=-1 в F major = Eb — ДИССОНАНС, убивает звучание!
       ❌ p4 >> bass(...) — bass = "пердящая" бас-гитара, звучит ужасно в вальсе!
       ❌ d2 >> play — 4-й слой, в вальсе достаточно только кика!
       ❌ НЕ добавляй pan= к p1>>bell — это усложняет рендер без пользы!
    ✅ РАЗРЕШЕНО ровно 3 слоя: p1(bell мелодия) + p2(pads гармония) + d1(кик)
    ✅ Правильный код (BPM=72, dur=0.5 — все ноты восьмые):
       ...
       p1 >> bell(
           midinote=[69,69,67,69,65, 60,60,60, 69,69,70,67,72,
                     72,65,65,70,70, 69,67,65,65, 69,69,67,69,65],
           dur=0.5,
           amp=0.85, sus=0.45, room=0.3
       )
       p2 >> pads([(0,4,7)], dur=3, oct=4, amp=0.2, room=0.5)  # F major аккорд, вальс 3/4
       d1 >> play("X..", sample=0, amp=0.3)                     # кик на 1-ю долю вальса
       # ← СТОП! Здесь код заканчивается. НЕ добавляй p3, p4, d2!
