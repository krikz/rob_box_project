# BLOCKERS — переопределение MiniMax endpoint для test bench

> **Статус:** blocker зафиксирован для текущего test-bench (kanban `t_df80ce58`).
> **Правило:** production-код (`src/rob_box_voice/`, `src/rob_box_llm/`) в рамках
> этой документационной задачи **не изменяется**.
