...
| W11 | 06-04 | Wave 4 | Single `perception_bridge.py` (~200 lines): UART → `/sensors/data`; **also includes W12 tests and partial cleanup of W10 leftovers** | `85cfd62e` (in `feature/harness-p0-foundation`) | **done** (retroactive, broader scope, **two-node architecture per ADR-0010**) | `06-04-PLAN.md` W11; `src/rob_box_perception/rob_box_perception/perception_bridge.py` (198 lines, ≤200 target ✓); `test_perception_bridge.py` (122 lines); commit also fixes `setup.py` entry_points AND removes `reflection/vision_stub/startup_greeting` from BOTH `internal_dialogue.launch.py` AND `internal_dialogue_docker.launch.py` + rewrites `setup.py` data-files (the launch cleanup that was supposed to be separate W10-cleanup); **scope decision** locked in ADR-0010 (`docs/adr/0010-perception-bridge-and-aggregator.md`) — `perception_bridge` (UART) + `context_aggregator` (9 ROS2 topics) co-exist because `mcp_server.py` is the downstream consumer of `/perception/context_update` |
...
| G2 | W11 SUMMARY doc | План 06-04 имеет `06-04-SUMMARY.md` | **Resolved** — `06-04-SUMMARY.md` существует, 249 строк, описывает W10+W11+W12 close-out | resolved (post ADR-0010 / `t_2ff2e9ad`) | `wc -l .planning/phases/06-harness-p0-finalization/06-04-SUMMARY.md` → 249; коммит `48312c7f` |
...
| G8 | `perception_bridge` registration & launch wiring | W11 acceptance: «`python -c 'from rob_box_perception.perception_bridge import PerceptionBridge; print("OK")'`», но неявно — нужен launch | **Resolved** — `setup.py` entry_points: `perception_bridge = rob_box_perception.perception_bridge:main` ✓; **ОБА launch-файла (`internal_dialogue.launch.py:43-67`, `internal_dialogue_docker.launch.py`) теперь стартуют `perception_bridge` + `context_aggregator` + `health_monitor`** | resolved (post ADR-0010 / `t_2ff2e9ad`) | `grep -n perception_bridge src/rob_box_perception/launch/*.launch.py` подтверждает (раньше было 0 совпадений) |
| G9 | `internal_dialogue.launch.py` после W10 cleanup | W10 acceptance: «no LLM, no reflection, no vision stub» | Текст launch-файлов уже исправлен в `85cfd62e` (commit комментарий: «launch files references to deleted nodes removed»); сами references на ноды удалены | resolved | `git show 85cfd62e` — `launch/internal_dialogue.launch.py` 72 ++++ / ----, `internal_dialogue_docker.launch.py` 99 ++++ / ----; **ADR-0010** фиксирует текущее состояние: ASCII-диаграмма в launch различает Phase 6 v2 (current: orphan publisher) vs Phase 7+ (target: bridge → aggregator) |
...
| Perception bridge tests | `pytest src/rob_box_perception/test/test_perception_bridge.py` | **13 passed in 0.11s** (verified `t_2ff2e9ad`) |
| Context aggregator tests | `pytest src/rob_box_perception/test/test_context_aggregator.py` | **30 passed in 28.24s** (verified `t_2ff2e9ad`) |
...
- **Plans / Summaries:** `.planning/phases/06-harness-p0-finalization/06-{01,02,03,04}-{PLAN,SUMMARY}.md` (06-01-SUMMARY, 06-02-SUMMARY, **06-04-SUMMARY** существуют; 06-03 — только PLAN).
- **ADR:** `docs/adr/00{01..10}-*.md` (10 файлов в `docs/adr/`; к harness относятся ADR-0001, ADR-0009; **новый ADR-0010** закрепляет perception scope decision — see §6).
