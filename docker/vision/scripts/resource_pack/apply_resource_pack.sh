...
# Асимметрия «у одного ресурса sha256 заполнен, у соседа пустой» тонет в
# сыром логе шага (issue замечен 21.09.2026 живым деплоем: retinaface OK,
# yolov8n — тихий WARN среди тридцати других строк). Под GitHub Actions
# `::warning::` попадает в панель Annotations, не только в raw-лог —
# emit её ЗДЕСЬ, а не в workflow, потому что шаг деплоя вызывает этот
# скрипт по ssh и его stdout/stderr — это и есть stdout/stderr шага
# (см. тот же приём в scripts/ci/fetch_hailort_wheel.sh:23-25 — ::error::
# в stderr, а не в stdout). Вне CI (голая Pi, ручной вызов) — no-op.
gha_warn() {  # $1 = title, $2... = message
    [ "${GITHUB_ACTIONS:-}" = "true" ] || return 0
    local title="$1"; shift
    echo "::warning title=${title}::$*" >&2
}
...
# Регистрирует ресурс, чей sha256 в манифесте пуст (целостность не
# проверена), в счётчике итога + громко под CI. Один вызов на каждое место,
# где раньше был только warn() — теперь асимметрия «у соседа sha есть, у
# этого нет» видна и в сыром логе, и в итоговой строке, и в Annotations.
unverified() {  # $1 = name, $2 = title, $3... = message
...
ensure_open() {  # $1 = индекс записи
    ...
    if [ "$present" -eq 1 ] && [ "$FORCE" != "1" ]; then
        ...
        if [ -z "$sha" ]; then
            ...
            unverified "$name" "Resource pack: sha256 не проверен" \
                "${name}: ${tpath} на месте, но sha256 в манифесте пуст — целостность НЕ проверена (план §13.4). Нужен эталон от владельца: ssh на робота/build-хост, sha256sum ${tpath}, вписать в manifest.yaml."
            NOOP=$((NOOP + 1))
            return 0
        fi
    ...
    if [ -n "$sha" ]; then
        ...
        unverified "$name" "Resource pack: sha256 не проверен" \
            "${name}: скачано с ${url}, но sha256 в манифесте пуст — целостность НЕ проверена (план §13.4)."
    fi
...
ensure_gated() {  # $1 = индекс записи
    ...
    if [ -z "$sha" ]; then
        ...
        unverified "$name" "Resource pack: sha256 не проверен (gated)" \
            "${name}: ${tpath} на месте (test -f прошёл), но sha256 пуст — целостность вендорного артефакта НЕ проверена. Эталон утрачен физически (план §4.3/§16.2) либо ещё не снят человеком."
        NOOP=$((NOOP + 1))
        return 0
    fi
...
# ---------------------------------------------------------------------------
echo "${TAG} ────────────────────────────────────────"
log "итог: установлено ${INSTALLED}, без изменений ${NOOP}, пропущено ${SKIPPED}, hard-провалов ${HARD_FAILS}, sha256 не проверен у:${UNVERIFIED:- (никого)}"
if [ -n "$DEGRADED" ]; then
    warn "объявленная деградация:${DEGRADED}"
    warn "это НЕ «всё хорошо» — потребители выше уйдут в stub (ADR-0018)."
fi
if [ -n "$UNVERIFIED" ]; then
    # Это НЕ деградация (ресурс встал, потребитель не деградирует) и НЕ
    # hard-провал (exit остаётся 0) — но это тихая дыра в целостности,
    # которая не должна выглядеть как «всё хорошо» рядом с ресурсом, у
    # которого sha256 заполнен и сошёлся. Итоговая строка выше уже считает
    # их числом; здесь — поимённо, одной строкой, которую не пролистать.
    warn "sha256 НЕ ПРОВЕРЕН (целостность неизвестна):${UNVERIFIED}"
    warn "это не деградация и не провал — это дыра в контроле целостности (план §13.4). Не воспринимать как «зелёно»."
fi
