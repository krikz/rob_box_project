...
docker exec oak-d bash -c 'source /opt/ros/kilted/setup.bash && ros2 topic list | grep oak'
echo ""

echo "4. Информация о сжатом RGB топике:"
docker exec oak-d bash -c 'source /opt/ros/kilted/setup.bash && ros2 topic info /oak/rgb/image_raw/compressed'
echo ""

echo "5. Информация о базовом RGB топике (если есть):"
docker exec oak-d bash -c 'source /opt/ros/kilted/setup.bash && ros2 topic info /oak/rgb/image_raw' 2>&1 || echo "Базовый топик отсутствует"
echo ""

echo "6. Проверка публикации (5 секунд):"
echo "   Проверяем /oak/rgb/image_raw/compressed..."
timeout 5 docker exec oak-d bash -c 'source /opt/ros/kilted/setup.bash && ros2 topic hz /oak/rgb/image_raw/compressed' 2>&1 || echo "   НЕТ ДАННЫХ или TIMEOUT"
echo ""
