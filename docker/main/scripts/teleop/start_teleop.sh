...
# Entry point is installed by CMakeLists.txt as `joystick_control_node.py`
# (install(PROGRAMS ...)), so `ros2 run` must use the .py name. The package
# build_type is ament_cmake — setup.py console_scripts are NOT generated.
