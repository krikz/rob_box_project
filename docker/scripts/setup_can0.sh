#!/bin/bash
# setup_can0.sh - Инициализация CAN интерфейса для VESC
# 
# Этот скрипт настраивает can0 интерфейс для работы с VESC регуляторами.
# Запускается на Main Pi перед стартом vesc_nexus контейнера.
