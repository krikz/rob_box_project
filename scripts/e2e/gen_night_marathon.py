...
act(
    ...
    [
        ...
        step(
            ...
            voice=GRISHA,
            # Issue #2779 AC3 — "must_not_say": имена и факты ДРУГИХ дикторов
            # каста не должны звучать незнакомцу. must_not_call здесь
            # используется нестандартно (как в n1007_no_leak) — это
            # substring-проверка по логу шага, а не список тул-имён.
            # register_speaker/Борис/Саш(а)/Спартак/пицц — именно то, что
            # утекло в живом прогоне 35734532425 (робот назвал незнакомца
            # Борисом и пересказал его факты).
            must_not=["register_speaker", "Борис", "Саш", "Спартак", "пицц"],
        ...
        step(
            ...
            # зарегистрирован. Раньше это можно было проверить только
            # глазами («читаем глазами»); теперь ассертим явно.
            must_not=["Гриш"],
            why="Оба зарегистрированных должны быть названы. Гриши в списке "
            "быть не должно — раньше это можно было проверить только "
            "глазами, теперь есть явный must_not (issue #2779 AC3).",
        ),
    ],
    [
