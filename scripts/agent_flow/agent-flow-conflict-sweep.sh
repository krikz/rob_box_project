...
if [ -n "${GH_REPO:-}" ] \
    ...
    echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] conflict-sweep: [MAINTENANCE] gate active on remote — skip" >&2
    exit 0
fi
if [ -n "${REPO_DIR:-}" ] && [ -d "$REPO_DIR" ] \
    && git -C "$REPO_DIR" show "${_branch}:${_file}" >/dev/null 2>&1; then
    echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] conflict-sweep: [MAINTENANCE] gate active locally in ${REPO_DIR} — skip" >&2
    exit 0
fi
unset _branch _file
