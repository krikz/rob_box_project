...
#   либо `--recover` для lift-фазы). Имеет три отличия:
#     1) сканирует LATEST summary задачи (task_runs.summary), а не свежесть
#        log-файла — то есть ловит «когда-то словили 429, retry-loop крутит»
#        даже если свежий лог уже без маркера;
...
#   task_runs.summary (последний run) содержит сигнатуру provider-exhaust
#   И статус сейчас НЕ 'blocked' (чтобы не дублировать).
#   Output: JSON-lines в $ACTIONS_FILE: {"action":"block","board":"...","task_id":"...","issue":"...","repo":"..."}
# ============================================================================
...
def summary_contains_exhaust(text: str) -> bool:
    if not text:
        return False
    low = text.lower()
    # Anti-patterns: recover/UNBLOCK messages (these are SAFE — block already lifted)
...
for db in sorted(glob.glob(f"{boards_dir}/*/kanban.db")):
    ...
    try:
        ...
        # Берём только non-done + non-archived задачи
        rows = con.execute("""
        ...
        for row in rows:
            ...
            if mode == "cancel":
                # Только если сигнатура в latest_summary и статус НЕ blocked
                if not summary_contains_exhaust(latest_summary or ""):
                    continue
                if status == "blocked":
                    # Уже заблокирован — пропускаем (идемпотентность)
                    continue
                if has_marker:
                    # Маркер уже есть → идемпотентность: не дублируем
                    continue
                ...
                if not has_marker:
                    continue
                # Доп. условие: marker-комментарий содержит root issue (#1193)
                # ИЛИ саммари восстановления; мы просто опираемся на наличие
                # SENTINEL_TAG (наш собственный marker).
                issue = extract_issue(body)
                repo = extract_repo(body)
                now_actions.append({
                    "action": "unblock",
...
apply_cancel() {
    ...
    local action="" board="" task_id="" issue="" repo="" latest_summary="" title=""
...
for k in ("action","board","task_id","issue","repo","latest_summary","title"):
    val = str(o.get(k, ""))
    val = val.replace(chr(39), chr(39)+chr(92)+chr(39)+chr(39))  # escape single-quote
    print(f"{k}=\x27{val}\x27")
')"

        log "→ $action  $board/$task_id  issue=#$issue  title='$title'"
...
карточка заблокирована по сигнатуре в \`latest_summary\` (\`$latest_summary\`).
