...
#      с key `nightly-review-<ISO-неделя>` (дедуп: см. ADR-0049 §6.1/ADR-0116).
...
#     найдёт ли ревьюер дефект. (ADR-0116 ревизия 08.09: первая версия
...
#     что ADR-0115 для обычных worker-отчётов).
#
# ENV:
...
# kanban-retro-create.sh (ADR-0116) внутри обращается к $GH_BIN под `set -u`.
...
### Персистентность (ADR-0116, ОБЯЗАТЕЛЬНО перед `kanban_complete`)
...
# найдёт ли ревьюер реальный дефект. Раньше (ADR-0116, первая версия) здесь
...
# Находки персистятся ревьюером через nightly-review-record.sh (ADR-0116,
