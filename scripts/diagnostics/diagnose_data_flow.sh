...
MAIN_RGB=$($SSH_CMD ros2@10.1.1.20 'docker exec rtabmap bash -c "/opt/ros/kilted/bin/ros2 topic list 2>/dev/null | grep -c color/image_raw"' 2>/dev/null)
MAIN_DEPTH=$($SSH_CMD ros2@10.1.1.20 'docker exec rtabmap bash -c "/opt/ros/kilted/bin/ros2 topic list 2>/dev/null | grep -c depth/image_raw"' 2>/dev/null)
