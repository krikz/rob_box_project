...
#:
#: ``theme_178`` (issue #2961, разбор 24.09.2026): «Theme» (Terminatorv
#: v2.0) — тема стартует с D (шестнадцатая перед восьмой E), опирается на
#: долгие F (целые ноты), тоника по слуху — ре минор, детектор даёт
#: F major (разрыв 0.28 в live-логе). Причина — не баг, а честная
#: неоднозначность параллельных тональностей: у F major и D minor
#: ОДИНАКОВЫЙ набор нот, а вес темы почти весь лежит на F (III ступень
#: ре минора = тоника фа мажора), тоника D звучит один раз шестнадцатой.
#: Общий тай-брейк «затакт, входящий в трезвучие кандидата, не затакт»
#: (см. :func:`~core.rtttl_compose._first_strong_note`) сократил разрыв
#: F/Dm с 0.38 до 0.18 (raw см. PR #2961), но не закрыл его. Перебор
#: других общих тай-брейков (сырая первая/последняя нота без привязки к
#: трезвучию кандидата, вес долгих нот на терции, поднятие
#: ``_FIRST_NOTE_WEIGHT``) — каждый либо не хватало по величине, либо
#: чинил theme_178 ценой регрессии на hallofth_2/mountainking/mozart40/
#: 4seasons_5/jinglebe_2/whitechr/thefirst из этой же таблицы (см. PR).
#: Раздельная симметричная эвристика без регрессий не найдена — как и у
#: toccata/jinglebe_5 выше.
