...
> 🔴 fix(voice #1252): список синхронизирован со `stt_node` (код+yaml) — все
> варианты должны совпадать 1:1. Исторический «робик» (потерян при 9ca7fb29,
> 21.02) возвращён. STT реально выдаёт кривые варианты («робок», «роберт»,
> «рыбок») — каждый из них должен давать ПРИНЯТО.

### Загрузка

```python
self.declare_parameter('wake_words', ['робок', 'робот', 'роббокс', 'робокос', 'роббос', 'робокс', 'роберт', 'рыбок', 'рома', 'бот', 'робо', 'роб', 'робик'])
