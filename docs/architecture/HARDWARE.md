...
| **Формат (аппаратный)** | MJPEG (preferred) / YUY2 (YUYV 4:2:2 fallback) |
| **Частота** | 5 FPS (оптимизировано для Vision Pi) |
...
| Топик | Тип | Частота | Назначение |
|-------|-----|---------|------------|
| `/ceiling_camera/image_raw/compressed` | `sensor_msgs/CompressedImage` | 5 Hz | Потолочное изображение (JPEG ~75 quality) |

**Примечание**: Камера публикует только compressed топик для минимизации нагрузки на Vision Pi. Используется MJPEG формат при поддержке камерой для прямой публикации без CPU overhead декодирования.
