...
**Режимы работы:**
- **1-channel firmware** — 1 канал обработанного аудио для ASR (распознавания речи). Аппаратный AEC чипа активен.
- **6-channels firmware** — 6 каналов (4 сырых микрофона + 2 playback-референса). **Аппаратный AEC НЕ применяется** — отдаются сырые данные. Это канонический режим для ROBBOX (см. [ADR-0012](../adr/0012-respeaker-6ch-channel-map-and-mixer.md)).

#### Прошивка и управление

**Конфигурация в ROBBOX (production, 2026-08+):**

| Параметр | Значение | Обоснование |
|---|---|---|
| Firmware | `6_channels_firmware.bin` | Доступ к 4 raw mics + 2 playback-референс |
| `mix_channels` | `[0,1,2,3,4,5]` (все 6) | A/B-победитель (PR #1093): иначе `yandex:empty` |
| Частота | 16 kHz нативно | Жёсткий лимит UAC1.0 + прошивки |
| Анти-эхо | НЕ на уровне микшера | VAD/wake-word gate (см. ADR-0012 §3.4) |

> **Важно.** В 6-канальном режиме **аппаратный AEC XMOS выключен** — каналы отдаются сырыми. Полная карта каналов и обоснование `mix_channels=[0,1,2,3,4,5]` — в [ADR-0012](../adr/0012-respeaker-6ch-channel-map-and-mixer.md). Предыдущая установка «исключить Ch5-6» (PR #1079) была откачена в `826fc128` — стабильно ломала Yandex STT.

**Доступные прошивки (reference):**
| Прошивка | Каналы | Описание |
|----------|--------|----------|
| `1_channel_firmware.bin` | 1 | Обработанное аудио для ASR (аппаратный AEC активен) |
| `6_channels_firmware.bin` | 6 | 4 сырых mic + 2 playback-референса (raw, без AEC) |
...
### 4.3. Телеопирование (Joystick/RC)

Робот поддерживает управление через SBUS/Serial-интерфейс (RC-пульт или джойстик через SBUS-ресивер).

| Параметр | Значение |
|----------|----------|
| **Протокол** | SBUS (через `/dev/ttyUSB*` или `/dev/serial*`) |
| **Пакет** | `rob_box_teleop` |
| **Контейнер** | `teleop` (Main Pi) |

> **⚠️ Известная проблема**: BLE-джойстик заблокирован kernel bug в Linux 6.14.0-raspi. Подключение по Bluetooth не работает — устройство обнаруживается, но `/dev/input/jsX` не появляется. Используйте проводное управление (SBUS/serial) или ждите исправления ядра. Статус: открытый upstream issue.
