...
Ни один из них не закрывает кейс «**биометрия говорит known, но с низкой confidence**». Это и есть брешь, которая валит n210 в 2 из 3 последних ночных прогонов (в третьем n210 прошёл тривиально — Борис не зарегистрировался из-за wake-слова, см. §1.3).
...
### 1.3 Три прогона — три разные причины, биометрия только в двух

3 последовательных прогона develop:

| run | failed | steps failed | причина |
|---|---|---|---|
| 35788126541 | 1/19 | n210_grisha_no_name | биометрия: Гриша опознан как Борис |
| 35781881888 | 2/19 | n208_memory_search_tea, n210_grisha_no_name | n210 — то же; n208 — флейк выбора tool у LLM (в 35788126541 шаг прошёл) |
| 35775984954 | 3/19 | n204_boris_intro_long, n209_recall_boris, n211_who_do_you_know | **wake-слово, не биометрия** |

**n210_grisha_no_name** провалился в **двух** прогонах из трёх, а не во всех трёх. В 35775984954 он прошёл
(`E2E_STEP n210_grisha_no_name OK`), но тривиально: Борис в том прогоне так и не зарегистрировался, в базе
был только один профиль, и назвать Гришу Борисом было не из кого.

Почему Борис не зарегистрировался — CI-лог 35775984954:

```
>>> STEP n204_boris_intro_long: ❌ NO_ACCEPT после 3 попыток
>>> TRANSCRIPT[n204_boris_intro_long]: ожидалось «Робот, добрый вечер, меня зовут Борис, …»,
    распознано «Робби, добрый вечер. Меня зовут Борис, …»
```

STT услышал «Робби» вместо wake-слова, тёрн не дошёл до LLM, `register_speaker('Борис')` не вызывался —
отсюда и n209/n211 («не помнит Бориса»). Это починено PR #2801 (`робби` в `wake_words.yaml`): в двух следующих
прогонах n204/n209/n211 зелёные. То есть n204/n209/n211 к бреши этого документа **не относятся**.

Настоящий повторяющийся отказ один — n210, и он **детерминированный**, а не флапающий: как только Борис
в базе есть, чужой голос Гриши получает его имя.
...
> **Оговорка: 0.85 закрывает этот конкретный прогон, но не корень.** Контрольный замер 22.09
> (`scripts/maintenance/voice_threshold_sweep.py`, девять студийных дикторов через боевой
> `preprocess_wav` + `embed_utterance`): у разных людей максимум сходства **0.879**, у одного
> человека между сессиями на роботе — 0.42–0.59. По журналу разбора 22.09 в прогоне 35781881888
> Гриша получил против Бориса **0.814** (в CI-логе этого числа нет, оно из лога робота). То есть
> ложные совпадения выше 0.85 возможны, а свои реплики между сессиями до 0.85 не дотянут. Корень —
> смена канала записи между сессиями, а не порог. Двухступенчатый порог — страховка для
> e2e, а не лечение; `identify_threshold=0.72` не трогать (см. §4.3).
