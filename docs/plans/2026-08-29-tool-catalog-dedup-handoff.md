...
## 8. Дубли в YAML-конфигах диалога

Вопрос был: «мне кажется там есть также дубли конфигураций для диалога».
Есть. Три штуки, все разной степени вредности.

### 8.1. Мёртвый монолит `voice_assistant.yaml` (`2b0bba11`)

ADR-0004 (issue #1004) заменил общий конфиг на файл-на-ноду: вложенные
секции `<node>:` в общем файле превращаются в dotted-параметры
`dialogue_node.llm_provider`, которых `get_parameter("llm_provider")` не
находит — нода молча работает на дефолтах, выглядя настроенной.

Деплой монолит выбросил, и **три файла об этом прямо пишут** —
`docker/vision/README.md`, `start_voice_assistant.sh`,
`voice_assistant_headless.launch.py`. А пакет продолжал его ставить
(`setup.py` глобит `config/*.yaml`), и `src/rob_box_voice/README.md` в
трёх местах учил оператора править именно его, как и
`training/README.md`.

Разъехался, разумеется: `wake_words` — старый список из 13 вариантов,
`llm_streaming: false` против живого true. Оператор, следующий README,
правил бы всё это впустую.

Удалён. Предохранители: `test_no_multi_node_yaml_in_the_config_dir` (ни
один файл в обеих config-директориях не может нести больше одного блока
`ros__parameters`) и `test_every_package_config_matches_a_node_name`.

### 8.2. `wake_words` — девять копий, три разных списка (`cd551181`)

| Где | Вариантов |
|---|---|
| `dialogue_node.py` declare default | 13 |
| `stt_node.py` declare default | 13 |
| `DialogueManager` литерал в конструкторе | 21 |
| `dialogue_text.DEFAULT_WAKE_WORDS` | 24 (упорядочен) |
| 4 × YAML (2 деплойных, 2 пакетных) | 21 |
| `voice_assistant_test.yaml` (e2e) | 13 |
| `test_wake_word_sync.CANONICAL_WAKE_WORDS` | 21 |

Последствия не гипотетические: **e2e-стенд гонял старые 13**, то есть не
слышал восьми вариантов, которые слышит робот («робэкс», «робакс»,
«рабокс», «рубокс», «роблокс», «рабок», «робак», «робекс» — все реальные
ослышки Yandex/vosk, ради которых деплойный список и расширяли).
Регрессия wake-word могла пройти e2e и упасть на роботе.

Теперь объявление одно — `dialogue_text.DEFAULT_WAKE_WORDS`. Это строгое
надмножество деплойного 21: добавляются `роб бокс`, `rob box`, `robbox`,
не убирается ничего. Его порядок тоже важен и раньше терялся:
`strip_wake_word` матчит leftmost-first, поэтому «роб» обязан идти после
«роб бокс», а копии в YAML лежали в произвольном порядке.

`test_wake_word_sync.test_yaml_wake_words_match_canonical` снят: он
проверял, что четыре YAML несут одинаковую копию, то есть сторожил
дублирование вместо того чтобы его убрать — и не покрывал ни дефолты нод,
ни e2e-конфиг, где разъезд и был.

### 8.3. Разъезд «пакет против деплоя»

Сравнил `<node>.yaml` попарно во всех нодах:

- `command_node`, `led_node`, `sound_node` — идентичны;
- `stt_node`, `tts_node`, `speaker_id_node` — расходятся только на
  `metrics_port` / `silero_warm_load` / `aec_mode`, и там, где параметра
  нет, он совпадает с дефолтом кода;
- `audio_node.speech_max_duration` — **15.0** в коде и в деплое, **12.0**
  в пакетном конфиге: лимит поднимали после «Речь отклонена: 12.76с» в
  двух местах из трёх. Тест `test_voice_command_is_under_vad_max_12s`
  хардкодил те же 12.0 и называл их «audio_node VAD max». Исправлено в
  `55bb3f25`, тест читает значение из деплойного конфига;
- `dialogue_node` — самый разъехавшийся: 2 конфликтующих значения, 7
  параметров только в пакете, 3 только в деплое.

### 8.4. Что осталось на решение

- **`faq_mode_enabled: true` в деплое** при пустом `faq_event_config_file`.
  Сейчас это no-op: `_load_event_profile` возвращает `{}`. Либо флаг надо
  выключить, либо положить профиль. Плюс у mcp_server отдельный
  переключатель — переменная окружения `FAQ_MODE_ENABLED` (сейчас off), и
  эти два могут расходиться.
- **9 параметров в e2e-конфиге, которых нода не объявляет**, то есть они
  молча игнорируются: `provider` (нода читает `llm_providers`),
  `streaming` (`llm_streaming`), `system_prompt` (`system_prompt_file` —
  и это путь, а не текст), `enable_fallback`, `max_iterations`,
  `silence_words`, `internet_available`, `max_history_messages`,
  `history_window_tokens`. `base_url` / `api_key` / `model` /
  `temperature` / `max_tokens` объявлены и работают, так что редирект на
  локальную Ollama жив. Переименование остальных изменит поведение e2e —
  это решение, а не правка.
- **`stt_node` fallback `has_wake_word`** (внутри `except ImportError`)
  матчит подстрокой, а не словом целиком — тот самый баг #1292, который в
  канонической версии починили. Срабатывает только при standalone-запуске
  без пакета.
