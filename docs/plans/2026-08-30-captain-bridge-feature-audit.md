...
| B2 | Drag/resize панелей: raycaster + XR trigger, десктоп — мышь | M |
...
Общий узел, который разблокирует сразу три ветки — **raycaster (B2)**:
без него нет ни панели режимов, ни выбора стрима, ни клика по карте.
Его стоит сделать первым, если браться за любую из них.
