...
> **РЕШЕНО 2026-09-16 (владелец проекта): вариант (б).** Вендорные
> артефакты Hailo не зеркалируются никуда — канонический экземпляр
> остаётся в `/opt/rob_box/vendor` на build-хосте, а незадокументированность
> закрывается инструкцией `docs/deployment/hailo-vendor-artifacts.md`
> (как получить, как извлечь `.so` из `.deb`, как засеять новый хост,
> что делать, когда пака нет). Вопрос лицензии Hailo на редистрибуцию
> тем самым снимается: новых способов распространения проект не создаёт.
> SPOF (потеря katana = повторный заход в Developer Zone) принят
> осознанно и зафиксирован в инструкции.
>
> Рассматривался и отклонён приватный пакет в GHCR: он устранял бы SPOF,
> но упирался в тот же вопрос редистрибуции. Публичный релиз с
> запароленным архивом отклонён отдельно — репозиторий `krikz/rob_box_project`
> публичный, поэтому пароль в стартовом скрипте публичный, и схема даёт
> обфускацию, а не контроль доступа.
...
## 16. Статус реализации, 21.09.2026

| Этап | Статус |
|---|---|
| 1 — манифест + скрипт | Сделан. |
| 2 — деплой HEF через пак | Сделан. Реального деплоя на робота ещё не было. |
| 3 — Vosk/Silero с хоста | Сделан. Требует проверки на железе — см. §16.4. |
| 4 — sha256 закрытых артефактов | Сделан ЧАСТИЧНО, см. §16.2. |
| 5 — уборка | НЕ делался намеренно: два download_*_hef.sh живут ещё один релиз-цикл как быстрый откат. |

### 16.1. Решение, которого не было в плане явно

Несовпадение sha256 — всегда hard, даже для `required: soft`. Отсутствие
ресурса — объявленная деградация (нода честно уходит в stub, ADR-0018), а
файл с другим содержимым — неизвестное состояние робота; в этом случае
скрипт не трогает ничего и возвращает 1. Это меняет поведение деплоя: шаг
«Ensure HEF models» раньше не падал никогда, теперь коды 1/2 валят его.

### 16.2. Этап 4: у половины записей эталон физически утрачен

Снято 21.09.2026 по ssh с katana (`/opt/rob_box/vendor`):

```
99da327b06a64ab8e5c87f09fc95cc68d2542d0f49b72369048ac003dc578e2a  hailort-4.24.0-cp310-cp310-linux_aarch64.whl
5fb8e54bb9945a3566930701af8dfc600edb5a15a7aca328b59cc18ce8d5a12c  libhailort.so.4.24.0
```

На Vision Pi снимать оказалось не с чего: каталога `/opt/rob_box/vendor`
там нет вовсе, а оба пакета уже установлены (`dpkg -l`: `ii hailort 4.24.0
arm64`, `ii hailort-pcie-driver 4.24.0 all`). Файлы `.deb` были применены и
не сохранены — зеркала у закрытых артефактов нет ни на роботе, ни в
репозитории, ровно как предупреждает §4.1/§4.3. Поля `sha256` у этих двух
записей остаются пустыми с явным объяснением: эталон вернётся только
повторной выгрузкой из Developer Zone.

Это же — аргумент к §4.3 (зеркало закрытых артефактов): сегодняшняя схема
уже потеряла два эталона из четырёх, и заметили мы это только потому, что
пошли их считать.

### 16.3. Deletion test (§8) — счёт положительный, и это записано честно

| ожидание §8 | факт |
|---|---|
| ступень скачивания в voice_base (40 строк) | удалено: 40 исполняемых строк в 0 |
| два download_*_hef.sh (174 строки) | НЕ удалены (Этап 5), к удалению готовы 193 строки |
| шаг деплоя (34 строки) | заменён, но стал длиннее: 18 удалённых против 82 добавленных в двух шагах |

Итого добавлено ~1538 строк нового (скрипт 612, манифест 234, тест 692),
удалено 56. Даже после Этапа 5 счёт останется положительным. Что реально
удалено — дублирование (две копии одного скрипта свелись в одну запись
манифеста), сетевая зависимость сборки voice-base и 113 МБ из слоя образа.
Строк стало больше, мест, где надо не забыть, — меньше.

### 16.4. Что обязательно проверить на железе, прежде чем считать Этап 3 готовым

```bash
# 0. чем распаковывать zip (в setup_*.sh unzip не ставится)
command -v unzip; command -v python3
# 1. прогреть руками ДО деплоя, чтобы 113 МБ качались не в окне деплоя
sudo bash ~/rob_box_project/docker/vision/scripts/resource_pack/apply_resource_pack.sh --only vosk-ru-small,silero-tts-v5-ru
# 2. второй прогон обязан быть no-op за меньше чем секунду
# 3. после деплоя — видит ли контейнер
docker exec voice-assistant ls -la /models/vosk-model-small-ru-0.22/README /models/silero/v5_ru.pt
docker logs voice-assistant 2>&1 | grep -i "Folder does not contain model files"   # ожидание: пусто
```

### 16.5. Что осталось

- Этап 5 (удалить два download_*_hef.sh, поправить комментарии
  `hailo_models.yaml`).
- ADR по §11.1 — свободный номер на 21.09.2026 был 0125, но перед
...
- §13.5 стал острее: прогрев `torch.hub.load` для silero удалён вместе со
  ступенью и ничем не заменён.

---

## 17. Дополнение после прогона на роботе, 21.09.2026

§16 писался до того, как скрипт впервые запустили на настоящей Vision Pi.
Прогон изменил три пункта.

### 17.1. CRLF ломал парсер молча

Манифест, приехавший с Windows-машины, `awk` на Pi (mawk) разобрал как ОДНУ
запись из десяти и спокойно продолжил. Механизм: пустая строка между
записями с одиноким `\r` не считается пустой, попадает в ветку «строка
нулевого отступа» и сбрасывает `in_res=0`, после чего весь остаток файла
игнорируется. Деплой в такой ситуации разложил бы часть ресурсов и
отчитался об успехе — ровно тот молчаливый PASS, против которого ADR-0018.

Починено (`sub(/\r$/, "")`) плюс регрессионный тест. Важная деталь для
следующего, кто будет это проверять: **на Windows баг не воспроизводится**,
тамошний awk сам съедает `\r`, поэтому негативный контроль гоняется только
в linux-контейнере с mawk.

### 17.2. Размеры в плане и в комментариях врали вдвое

Измерено на роботе: `v5_ru.pt` — **145 382 211 байт (139 МиБ)**, а не
«~68 МБ»; распакованный Vosk — 88 МиБ; суммарно на диске **~227 МиБ**, а не
«113 МБ». Цифра «113 МБ» кочевала из комментария `voice_base/Dockerfile` в
план и в compose. Исправлена везде.

### 17.3. sha256 у открытых моделей больше не пустые

Vendor их не публикует, поэтому записано то, что реально приехало и
заработало: vosk-архив — `961d5ff9…a46d4` (снято curl'ом на katana),
silero — `7ba04d42…97ab4` (снято с файла, разложенного скриптом на Pi).
Смысл поля — заметить обновление апстрима, а не подтвердить подлинность;
формулировка закреплена в ADR-0125 §2.6.

Проверка замкнулась: после простановки sha прогон на роботе дал
`OK no-op — на месте, sha256 совпал`.

### 17.4. Что из §16.4 уже сделано, а что нет

- **Сделано:** `unzip` на Pi проверен (его нет — сработал фолбэк
  `python3 -m zipfile`), модели разложены, идемпотентность подтверждена
  вторым прогоном, sha сверены.
- **НЕ сделано:** деплой на робота не запускался, поэтому не проверено, что
