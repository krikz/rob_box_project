...
dialogue_node  <--/voice/speaker/epithet_request--------+
   ...
   слой 2: LLM придумывает кличку (2–4 слова, §10.5)
      |
...
### 10.5 Изменено: кличка LLM — 2–4 слова (issue #2887, решение Шифу 23.09.2026)

В v1 LLM-кличка была **ровно одним словом**. Обоснование брали из §4.2:
«Весело-Угрюмый-Кентавр» — пример того, что LLM без рамки выдаёт
странное. 23.09.2026 товарищ Шифу решил, что клички должны быть
«покруче»: **2–4 слова**, например «Мудрый собеседник-спортсмен» или
«Ночной паяльщик моторов». Правило «одно слово» отменено. Рамка
осталась, просто теперь она другая:

- **Форма.** 2–4 слова, первое с большой буквы, только буквы; дефис
  внутри слова допустим и отдельным словом не считается. Всего не больше
  48 символов (`LLM_EPITHET_MAX_LEN`). 1 слово и 5+ слов отклоняются, а
  **не обрезаются**: обрывок фразы хуже словарной клички, которая уже
  лежит в БД. Ответы-отказы («Не могу придумать») отсекаются по стоп-словам.
- **Смысл.** Фильтр «незнакомца» (#2864) проверяет каждое слово и каждую
  часть через дефис («Мудрый незнакомец» — отказ). Для основы «гост»
  добавлены исключения («Гостеприимный», «Гостиничный»): в многословной
  кличке они встречаются, а про гостя ничего не говорят.
- **Имя.** Если в кличке есть имя знакомого человека (своё или чужое),
  она отклоняется: «Борис-путешественник» у Саши LLM прочла бы как
  упоминание Бориса.
- **Занятость.** Сравнивается вся кличка целиком, без учёта регистра, ё и
  лишних пробелов (`normalize_epithet`). Одно общее слово занятостью не
  считается.
- **Промпт.** Просит 2–4 слова из того, что человек рассказал о себе.
  Словарные кандидаты передаются как слова-ориентиры по теме, а не как
  образец формы. `max_tokens` поднят с 16 до 48.
- **Словарный слой остаётся одним словом.** Он нужен секунды, пока нет
  ответа LLM, и дольше только офлайн. Собирать его из частей без
  морфологии нельзя: прилагательное не согласуется с родом
  («Тихий Ладья»). Бонус: словарная кличка (1 слово) не может совпасть с
  LLM-кличкой (2–4 слова).
- **Причины отказа в логе** (#2886): `empty`, `word_count`, `invalid`,
  `stranger`, `name`, `taken_by=<id>`.

Потребители длинной строки проверены по коду: `<epithet internal="true">`
в контексте LLM (там только буквы, пробел и дефис — экранировать нечего),
`Acquaintance.epithet` в harness (длину не ограничивает),
`speaker_db_admin.py list` (колонку расширили с 14 до 48). Кличка не
озвучивается (§5.3). В TG-боте и GUI поля `epithet` в коде нет.
