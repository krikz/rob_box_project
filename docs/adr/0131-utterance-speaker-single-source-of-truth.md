...
**Issue:** [#2829](https://github.com/krikz/rob_box_project/issues/2829); пересмотр §2.3a — [#2862](https://github.com/krikz/rob_box_project/issues/2862)
...
* Новый топик **`/voice/stt/utterance`** (`String`, JSON `{"utterance_id": "..."}`), публикуется `stt_node` для КАЖДОЙ принятой ReSpeaker-фразы, ПЕРЕД `/voice/stt/result` (тот же порядок гарантий, что уже используется для `/voice/stt/speaker` — публикатор один и тот же поток, публикации строго последовательны). В отличие от `/voice/stt/speaker` (публикуется только когда есть Yandex speaker_tag — на Vosk-фоллбэке пропускается), `/voice/stt/utterance` публикуется всегда: `dialogue_node` должен иметь id для каждой фразы, а не только для тех, где Yandex дал тег.
  **Пересмотрено в #2862 (см. §2.3a):** порядок публикации НЕ означает порядок обработки у подписчика — id может прийти после текста.
...
### 2.3a Связывание id с фразой по тексту (issue #2862)

Исходная схема «id публикуется ПЕРЕД текстом → один pending-слот в `dialogue_node`» на роботе не выдержала (develop `a002c65`, E2E акт 2 run 35886659057): DDS не упорядочивает доставку между РАЗНЫМИ топиками, а `_on_stt_utterance` и `_on_stt` крутятся в `ReentrantCallbackGroup` на `MultiThreadedExecutor`. Если текст обработан раньше id — ход получал `None` (`register_speaker` → `no_utterance_context`), а опоздавший id оставался в слоте и доставался СЛЕДУЮЩЕЙ фразе (backlog-фраза `1a2f742eb17c` ушла в ход «Робот, продолжаю говорить…») — т.е. биометрия и регистрация чужого голоса.

Рассмотрены варианты:

* **(а) id+text в `/voice/stt/utterance`, сопоставление по тексту + короткое ожидание** — выбран. Тот же приём, что `_speaker_by_text` (#1077), контракт `/voice/stt/result` (plain text) не меняется.
* (б) только короткое ожидание в `_on_stt` при одном слоте — не решает «id фразы N пришёл после текста N+1»: слот не знает, чьей фразе он принадлежит.
* (в) единое JSON-сообщение вместо `/voice/stt/result` — ломает контракт для telegram/perception/GUI/harness (см. §2.3), не инкрементально.

Реализация — `core/utterance_binding.py::UtteranceIdBinder`:

* `stt_node._publish_utterance_id(audio_bytes, text)` публикует `{"utterance_id", "text"}`, где `text` — ровно та строка, что уходит в `/voice/stt/result`.
* `_on_stt_utterance` → `offer(text, id)`; `_on_stt` → `claim(raw_text, timeout)` ДО `SttAdmission`: фраза, которая не станет ходом (backlog no_wake_word, silenced, rejected), всё равно поглощает СВОЙ id.
* Id возвращается только claim'у с тем же текстом → не может достаться другой фразе. Если текст обогнал id — `_on_stt` ждёт до `_UTTERANCE_ID_WAIT_SEC = 0.5с` (оба топика публикуются подряд из одного потока, расхождение — планировщик executor'а, мс). Не дождались → `None` (честный unknown) + tombstone: опоздавший id выбрасывается при приходе.
* Telegram-ввод (`[TG:…]`) не ждёт (у него id не бывает). Текст без id вовсе (GUI/bench/инъекция харнесса) платит ≤0.5с один раз и идёт с `utterance_id=None` — раньше такой текст мог забрать чужой id из слота.
* TTL 30с и потолок 64 ключа — только ограничение памяти; корректность держится на совпадении текста.

Остаточный риск (не закрыт): ДОСЛОВНО одинаковый текст дважды подряд, id первой фразы опоздал >0.5с И пришёл уже ПОСЛЕ id второй фразы. Тогда tombstone первой фразы съест id второй, а опоздавший id первой достанется второму ходу. Id различить нельзя (хеш аудио), так что это ограничение схемы; требует совпадения трёх маловероятных условий. Если id первой опоздал, но пришёл раньше id второй — всё корректно (опоздавший выброшен, второй ход получает свой id).
...
* ~~Порядок `/voice/stt/utterance` → `/voice/stt/result` гарантирован только тем, что оба публикуются из одного потока~~ — риск реализовался на роботе (#2862), закрыт связыванием по тексту (§2.3a). Порядок топиков больше не важен.
* ~~Telegram-путь съедает id голосовой фразы~~ — после #2862 Telegram-ввод забирает id только по своему тексту и без ожидания, голосовой id он не трогает.
