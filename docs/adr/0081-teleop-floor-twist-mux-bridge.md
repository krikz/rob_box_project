...
- [x] Заведена карточка `t_9d33e8ca` (backend) на реализацию с
      ...
      «Дрейф», зафиксирован в 4 пунктах (D1, D2, D3, D4); решение
      владельца 2026-09-09 08:02 UTC — **вариант (a) править PR #2205**,
      ADR-0081 амендить не нужно. Реализация правок — отдельная карточка
      backend (наследник `t_9d33e8ca`), открывается по решению Шифу.
...
ADR-0081 в четырёх точках (D1, D4, D2, D3 — D1 и D4 связаны, см.
таблицу ниже). Это нужно явно зафиксировать — Шифу должен
...
| 2026-09-09 08:02 UTC | Решение Шифу в PR #2200 (GOODWORKRINKZ, «Ответ на §7: вариант (a), и это не выбор, а исправление дефекта») | **Вариант (a) — править PR #2205.** Амендить ADR не нужно (ложная развилка). §7 дополнен пунктом **D4** (`cmd_vel_quest` priority 40→90, связан с D1). Итоговый список правок PR #2205: D1+D4 приоритеты, D2 watchdog, D3 de-dup. Реализация — отдельная карточка backend. |
