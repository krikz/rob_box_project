...
| Поле | Значение |
|---|---|
| Статус | **Accepted** (решение владельца по §7 принято 2026-09-09: вариант (a)) |
| Дата | 2026-09-09 |
...
- [x] Решение по подключению/удалению принято владельцем — вариант (a), см. §7.
...
## 7. Решение владельца (принято 2026-09-09)

Этот ADR — **рекомендация архитектора**, не финальное решение.
Карточка #2245 явно говорит «Тип: решение владельца». Владельцу задан
вопрос («нужно ли «стой!» через прерывание TTS на середине»); ответ —
делегировать разбор архитектурному анализу (`codebase-design`:
модуль / шов / тест на удаление). Разбор и решение — ниже.

**Тест на удаление.** Убрать `ReflexLayer` + мост прямо сейчас — и
никакое поведение ниже по стеку не исчезает: §1.1 (в)-(г) показали,
что он подписан на шину **другого** `TaskScheduler`, в который никто
не кладёт задач. Сложность не «всплывает у вызывающих» — она просто
исчезает. Модуль не набрал глубины (не было ни одного правильно
подключённого вызывающего), удаление ничего не стоит.

**Почему вариант «(c) починить проводку» не купил бы возможность.**
`EventBus.cancel(task_id)` — шов на уровне «отменить **будущее**
выполнение задачи, которая ещё не забрала аудио-хардвер». «Стой! на
середине слова» нужен другой шов — внутри `tts_node`, там, где чанк
уже льётся в железо и нужен сигнал «прекратить *эти* байты сейчас».
Даже потратив 400-600 LOC на §3.1 (поднять шину до общего assembly),
`TaskScheduler.cancel` не достаёт до места, где решается задача — это
не «почти работает, не подключено», это шов не там, где нужна
возможность. Ремонт проводки к модулю, который структурно не может
отдать нужную поведенческую глубину, — работа на неправильный модуль.

**Почему «(b) priority-врезка» — не запасной вариант, а уже готовое
решение.** `_normalize_tts_priority` (ADR-0066 §8а.3) сидит на
правильном шве: «что играть **следующим**» — интерфейс `tts_node`,
которому и положено решать судьбу очереди. Он не тянется к
already-in-flight байтам, поэтому не воскрешает v36-регресс (§3.2).
Никакой новой работы вариант не требует — это существующая глубина
`tts_node`, а не альтернатива, которую нужно строить.

**Решение (принято 2026-09-09): вариант (a).** Не нужен, удаляем
ADR-0086 как есть. Если когда-нибудь понадобится настоящее прерывание
на середине — это отдельный ADR про новый шов **внутри `tts_node`**
(сигнал отмены на уровне потока воспроизведения), а не воскрешение
`ReflexLayer`: он строился под другой контракт (отмена запланированной
задачи) и не решает прерывание уже играющего байтового потока даже
в подключённом виде.

ADR-0054 архивируется немедленно (revert из git восстановит при
нужде). `command_node.yaml:17` (`enable_reflex_layer: true`) и
упоминания рефлекса в `CONTEXT.md` вычищаются полностью — половинчатая
чистка (флаг остаётся, код исчезает) хуже, чем полная чистка: следующий
воркер увидит рабочий с виду флаг и включит его в пустоту.
