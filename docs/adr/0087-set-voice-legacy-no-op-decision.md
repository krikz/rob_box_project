...
| Поле | Значение |
|---|---|
| Статус | **Accepted** (решение владельца 2026-09-09: вариант (a) — удалить) |
| Дата | 2026-09-09 |
...
## 5. Решение владельца (принято 2026-09-09)

**Выбран вариант (a) — удалить.** Аргументация в §2-§4 достаточна:
заглушка под `/dialogue/control` не получила ни одной карточки за
~1 неделю с ADR-0080, у пути нет владельца, а прецедент ADR-0086
уже показал, к чему приводит отложенный мёртвый код без плана. Ниже
исходные варианты — для истории.

Реализация — отдельная карточка (issue, план §4 этого ADR).

Варианты, рассмотренные до решения:
