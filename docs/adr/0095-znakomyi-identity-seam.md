# ADR-0095: «Знакомый» — единый шов идентичности человека взамен трёх несвязанных ключей
...
ADR-0089 Phase 2 ОБЯЗАН реализовать `FaceIdentityAdapter`, следующий этому контракту; в противном случае карточка ADR-0095 остаётся `Proposed` и Phase 2 блокируется на ревью.
...
def speaker_scope(speaker_id: str) -> str:
    ...
    Начиная с ADR-0095, ``speaker_id`` — это стабильный биометрический
    UUID (или любой стабильный ключ, который адаптер шва IdentitySeam
    назначил этому человеку). До ADR-0095 сюда передавался Yandex
...
| 7 | Задокументировать контракт шва как обязательную точку интеграции для ADR-0089 Phase 2 | §5.3 | Этот ADR (документ) + комментарий в `docs/adr/0089-ai-hat-plus-deployment.md:300` (правка в этом PR или в Phase 2 PR): «arbitration гол+лицо — через IdentitySeam, см. ADR-0095 §5.3». |
...
- ADR-0037 §2.5 не переоткрывается; в этом PR добавляется одна строка в его changelog: «§2.5 scope=speaker:<...>: с ADR-0095 в <...> подставляется биометрический UUID (было — Yandex tag)».
...
def test_threshold_values_match_code():
    """ADR-0095: проверка ЗНАЧЕНИЙ порогов, не только имён."""
...
**Решение:** этот ADR первым. ADR-0055 про физическое расположение файлов БД, и ему всё равно, какой id в столбце — стабильный или нет. Наоборот, после ADR-0095 ADR-0055 становится проще: переносить нужно одно key-пространство (биометрический UUID), а не три.
...
**Да.** `VoiceIdentityAdapter` оборачивает, не модифицирует. Если в реализации окажется, что нужны новые методы в `SpeakerDatabase` (например, `get_speaker_with_profile()`), они добавляются **минимально**, с тестами, и ADR-0095 остаётся «обёрткой», а не «переписыванием».
...
- Не меняем `mcp_server._on_speaker_result` — он уже сохраняет полный UUID в fallback, после ADR-0095 это станет основным путём.
