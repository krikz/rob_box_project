# ADR-0116: nightly-review persistence — JSONL + ISO-week dedup + worker-recorded findings

> Ранее фигурировал как ADR-0079 в коммите `d149e7897` (PR #2177). Перенумерация 2026-09-15: см. issue #2582 — сосед `0079-preview-voice-synthesis.md` влит раньше (`31c71f641`, PR #2173) и остался на 0079.
...
| Зависимости | ADR-0049 (ночной ревью-цикл), ADR-0018 (культура честности), ADR-0115 (worker-report паттерн, которому здесь следуем) — **реальный ADR-номер этого документа — 0079** (0078 уже занят ретро t_c4cd1f74). |
| Родители | ADR-0046 / ADR-0047 (примеры повторов симптомов 8 и 6 раз — то, что эта архитектура предотвращает) |
| Связанные | `agent-flow-nightly-review.sh`, `kanban-retro-create.sh`, `nightly-review-record.sh`, `kanban-report-write.sh` (ADR-0115), `validate_honesty.sh` |
...
**Исправление — тот же паттерн, что ADR-0115 уже использует для обычных
...
паттерн, что `kanban-report-write.sh` для ADR-0115:
...
- **Compliance ревьюера**: ничто технически не заставляет LLM-ревьюера
  ...
  `warn_no_worker_report` из ADR-0115) — не реализован в этом ADR,
  кандидат в follow-up, если compliance окажется низкой (см. метрику в
  ADR-0115 — тот же вопрос для worker-отчётов вообще).
...
`nightly-review-record.sh`, вызываемый ревьюером (паттерн ADR-0115).
