...
## 10. v2 deltas (PR z-devops/t_d5a31e86-...)

PR #1634 (CLOSED 25.08 без merge) реализовал это ADR. Но Шифу в фидбэке указал два критических дефекта, которые исправляет **v2** (этот коммит):

### 10.1 Cron-spam bug (защита от мусорных коммитов)

**Проблема v1**: workflow_trigger `schedule` запускал `commit-versions` job. Внутри `SOURCE_SHA="${GITHUB_SHA}"` (fallback когда build не передал source_sha). На cron-триггере `GITHUB_SHA` = SHA develop HEAD в момент cron'а — это НЕ реальный SHA билда, а develop HEAD. Скрипт создавал новый коммит `ci: main SHA tags → dev-XXXXXXX [skip ci]` где XXXXXXX = develop HEAD SHA. Результат: каждый час → новый мусорный коммит в develop.

**Решение v2**:
```yaml
commit-versions:
  if: success() && (github.event_name == 'workflow_call' || (github.event_name == 'workflow_dispatch' && inputs.source_sha != ''))
```
Cron не инициирует commit. Только явный вызов из build-workflow или dispatch с валидным source_sha. Mirror остаётся идемпотентным при cron (повторный тег-пуш безопасный).

### 10.2 Fail-OPEN dedup (Шифу: «чот падает — пуш не должен происходить вообще»)

**Проблема v1**: `push-image-versions.sh` имел fail-OPEN семантику — если `gh api` падал (network blip, auth, rate-limit, GitHub 5xx), скрипт шёл дальше и пушил. Шифу: «**чот падает и по прежнему в процессе есть пуш образов в гит убери его из локальной сборки!!!**». Fail-OPEN создавал duplicate commits при сетевых проблемах.

**Решение v2**: добавить fail-CLOSED по умолчанию:
- `gh api` failure → return 3 (skip push) → exit 0 (caller treats as success)
- Новый env `IMAGE_VERSIONS_FAIL_CLOSED_DEDUP=1` (default)
- Opt-out через `IMAGE_VERSIONS_FAIL_CLOSED_DEDUP=0` (legacy fail-open для диагностики)
- Rationale: duplicate SHA-tag commit хуже чем пропущенный push (cron retry через час)
- Unit-тесты: 5 кейсов (default + opt-out + 3 базовых сценария dedup)

### 10.3 Физическое удаление job (не comment-out)

PR #1634 v1 заменял `update-image-versions:` на `trigger-update-image-versions:` job, что в diff'е выглядит как удаление + добавление. Но lyrical-merge в t_dec05b13 показал, что **закомментированный** job может быть восстановлен stale-feature-branch'ем. **v2 явно физически удаляет** все упоминания старого job из build-workflow, оставляя только новый trigger-call и комментарий-маркер «удалено в #1630 v2».

### 10.4 Bugfix: `return 1` в if-блоке вне функции

PR #1634 v1 содержал bash-ошибку: `return 1` внутри `if [[ ! ... ]]; then ... fi` блока (вне функции). В bash `return` допустим только внутри функции или sourced скрипта, иначе → ошибка `return: can only return from a function or sourced script`. v2 заменил на `exit 2`.

### 10.5 Дополнительные уточнения v2

- `permissions: {}` в build-workflow (раньше было `contents: write` — больше не нужен, т.к. build не делает git push)
- IMAGE_VERSIONS_GITHUB_REPO override для unit-тестов (позволяет тестировать dedup с file:// remote)
- Строгая проверка `^https://github.com/(.+)/(.+)\.git$` (раньше было `[[ "$GH_REPO" != *"/"* ]]` — false-positive на любой URL со слэшем)

### 10.6 Acceptance criteria v2

- [x] Новый workflow `.github/workflows/L-Update Image Versions.yml`
- [x] `update-image-versions` job **физически удалён** из обоих build-workflow
- [x] `permissions: contents: write` удалён из build-workflow (больше не нужно)
- [x] Build делает только `docker push` в local registry
- [x] `commit-versions` НЕ запускается на cron (if-guard)
- [x] `push-image-versions.sh` fail-CLOSED на gh api failure (default)
- [x] 5 unit-тестов dedup зелёные (default + opt-out + 3 базовых)
- [x] pre-existing fail-fast test зелёный (TEST-NET-1 → 122s exit 1)
