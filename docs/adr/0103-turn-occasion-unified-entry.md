# ADR-0103 — «Повод» (Occasion): единый шов «можно ли заговорить»

> Ранее фигурировал как ADR-0101 в коммите `e7c9abc6` (PR #2575). Перенумерация 2026-09-15: см. issue #2582, ADR-AF-0068 (collision-fix по ADR-AF-0030 §2.4).
...
|---|---|---|
| Осиротевший `startup_greeting_node.py` (есть в дереве, нигде не запускается; единственный импортёр — собственный тест) | `src/rob_box_voice/rob_box_voice/startup_greeting_node.py`, `src/rob_box_voice/setup.py:51-66`, оба launch-файла | **Отдельная worker-карточка** `chore(voice): удалить осиротевший startup_greeting_node.py`. Решение: удалить или зарегистрировать. **Не блокирует** ADR-0103. |
...
**Принять** ADR-0103 со следующими условиями:
