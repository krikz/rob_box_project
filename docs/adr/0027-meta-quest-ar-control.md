...
> который (а) отдаёт статический WebXR-клиент (Three.js + WebXR Device API), (б)
...
> сети робота. Никакого мульти-юзера и удалённого доступа на этом этапе — это
> research-фича для лаборатории.
...
Три режима — это **не** три параллельных пайплайна, а один параметр
...
quest_ttts, quest_stt}`. Это **мини-фича в dialogue_node**, не отдельный
сервис — голосовой пайплайн у нас уже есть, и дублировать его ради Quest
было бы безумием.
...
8. WebXR-клиент: статический билд Three.js + WebXR Device API, source
