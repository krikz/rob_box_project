...
| Родители | ADR-0018 (честность), ADR-AF-0026 (recovery contract — worker отвечает за следствие), `t_45db74ad` (Phase 1 retro), `t_5055c13c` (Phase 2 retro) |
...
- Слово **«harness»** в репозитории имеет ДВА разных значения: (а) рантайм-пакет `rob_box_harness` (ADR-0001, ADR-0009 — TTSProvider contract); (б) AI-харнес агентов / Hermes (ADR-AF-0027 «анализ внешнего опыта построения AI-harness», ADR-AF-0028 «subagent-review», ADR-AF-0024 `OUT_DIR/verdict.txt`). **Классифицировать по слову в имени нельзя — только по содержанию**.
...
```markdown
См. [ADR-0022 §4.6](../0022-process-e2e-done-gates.md) (e2e done gates).
...
| **Удалить дубликаты автоматически (most recent wins)** | Уничтожает решение. ADR — задокументированное решение, а не временный файл. Авто-удаление без ревью — нарушение ADR-0018. |
