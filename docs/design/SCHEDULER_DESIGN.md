...
8. **Q6 (reflex, §8.10.8)** — логировать ли reflex-events в LLM feedback? Предложение: да, всегда (стоимость нулевая, выгода — LLM не «виснет»). PM подтверждает.
9. **Q7 (reflex + AWAITING, §8.10.8)** — reflex-«стоп» это ответ на AWAITING или отдельная операция? Предложение: вариант B (отдельная). PM подтверждает.
