...
> **СТАТУС (2026-08-13):** фазы 1–4 **приземлены в develop** (см. §11.6 SCHEDULER_DESIGN.md v6,
> коммиты `83947d0f`…`806eb146`, `74da1902`). Карточки #0–#7 ниже — исторический план нарезки,
> почти весь исполнен. **Оставшаяся работа — одна карточка W7** (интеграция scheduler в
> `dialogue_node`): код-точный план в `docs/design/W7_INTEGRATION_PLAN.md` (v1.0, 2026-08-13).
> Issue #968 переоткрыт пользователем («Он сейчас не работает!!») — W7 закрывает жалобу.
...
- ~~**Q5.1** (§14 #7): какой вариант A'1/A'2/A'3 рендера AWAITING-вопроса во время песни~~ — **✅ ЗАКРЫТ (v6, 2026-08-12, architect): вариант A'2 — врезка вопроса на ближайшей границе voice-сегмента (§8.11 SCHEDULER_DESIGN).**
- ~~**Q3** (§14 #3): владелец ROS-топика `/harness/task_events` — отдельный node или publisher из dialogue_node~~ — **✅ ЗАКРЫТ (v6, 2026-08-12, architect): publisher внутри `dialogue_node`, отдельный node не нужен (§14.1 SCHEDULER_DESIGN).**
- **Q10** (§14 #10): ROS-топик vs прямой вызов для reflex-events (затрагивает #5) — **открыт, решается после старта прототипа фазы 1.5 (аналитик).**

Эти вопросы блокируют реализацию, не проектирование. Можно стартовать карточки #0, #2, #6 без них. С закрытием Q5.1/Q3 в v6 сняты блокеры карточек #3 и #5 (остаётся Q10 для #5).
