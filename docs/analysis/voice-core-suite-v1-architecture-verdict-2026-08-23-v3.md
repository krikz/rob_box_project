...
| Цель | Подтвердить verdict v3 после +7 коммитов в develop И расширить его архитектурным предложением ADR-RT-0069 (topic-injection test path), которое убирает зависимость e2e от сломанного audio-bridge 249↔21 |
| Связано | issue #1506, verdict v2 (`voice-core-suite-v1-architecture-verdict-2026-08-23.md`), verdict v3 (этот же файл, v3-секция ниже), PR #1555 (merged), PR #1556 (merged — pre-flight), **PR #1559 (OPEN, MERGEABLE, CI 8/8 — добавлен ADR-RT-0069)**, ADR-0022 §4.6, ADR-RT-0068 (accepted), **ADR-RT-0069 (proposed — этот PR)** |
| Вердикт | **APPROVE v3.1** — verdict v2/v3 подтверждён + добавлен ADR-RT-0069 как архитектурный разблокер hardware-bridge |
| Re-validation | **v3.1 (23.08 ~17:50) — APPROVE подтверждён** на полном develop tip + ADR-RT-0069 как мост к зелёному e2e |
...
- ADR-RT-0068 — `accepted` (коммит `86889ee1`, 22.08 — `harness verdict SoT pull`)
...
| 4 | ADR-RT-0068 (music-aware intent priority gate) — accepted в develop | ✅ ACCEPTED | `docs/adr/0024-music-aware-intent-priority-gate.md` — статус Accepted, 22.08 |
| 5 | dj02_stop_music защищён ADR-RT-0068 (Nav2-коллизия) | ✅ COVERED | verdict v2 §3.1 «архитектурное замечание» — dj02 chain-failure валиден и наблюдаем |
...
ADR-0022 §4.6 + ADR-RT-0068 (accepted) обеспечивают корректную семантику
...
## 7. Расширение v3 → v3.1: ADR-RT-0069 «topic-injection test path»
...
### 7.2 Что нового в v3.1 — ADR-RT-0069
...
**ADR-RT-0069** (этот PR, отдельный файл `docs/adr/0026-voice-e2e-topic-injection-test-path.md`):
...
**Архитектурный долг по issue #1506 = 0 + ADR-RT-0069 как мост к e2e-PASS.**

- Verdict v2/v3 (PR #1555/#1559) — APPROVE подтверждён.
- ADR-RT-0069 (этот PR, **docs-only**) — **proposed**, готов к Approval Шифу.
- Hardware-bridge 249↔21 — больше **не блокер** для закрытия issue #1506
  при условии ADR-RT-0069-Accepted.
...
- `gh pr view 1559` = OPEN, MERGEABLE, CI 8/8 (verdict v3.1 + ADR-RT-0069)
