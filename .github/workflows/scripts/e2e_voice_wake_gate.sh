...
wake_gate_cleared_since() {
    ...
    # bug(run 35658231116, 22.09.2026): grep -F искал "✅ ПРИНЯТО: Робот", а
    # stt_node.py:923 печатает `✅ ПРИНЯТО ({source}): {text}` — с тегом
    # источника перед двоеточием (`✅ ПРИНЯТО (respeaker): Робот, ...`).
    # Тег появился в 6e016325f (wake-роутер, #2011), фиксированную строку
    # тогда не поправили. На живом логе прогона 35658231116:
    #     grep -F "✅ ПРИНЯТО: Робот"                → 0 совпадений
    ...
    #
    # Квантификатор `?` применяется к группе из ASCII и пробела, а не к
    # кириллическому символу — исходный unicode-quirk, из-за которого здесь
    # выбрали -F (Робок?с), так не воспроизводится.
    printf '%s' "$logs" | grep -qE "✅ ПРИНЯТО( \([^)]*\))?: Робот" \
        || printf '%s' "$logs" | grep -qE "✅ ПРИНЯТО( \([^)]*\))?: Робокс"
}
