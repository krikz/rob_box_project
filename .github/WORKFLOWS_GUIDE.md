...
Проект использует 4 GitHub Actions workflow для сборки Docker образов:

1. **build-all.yml** - полная сборка всех образов
2. **build-base-images.yml** - базовые образы (ros2-zenoh, rtabmap, depthai, pcl)
3. **build-main-services.yml** - сервисы Main Pi (robot-state-publisher, rtabmap)
4. **build-vision-services.yml** - сервисы Vision Pi (oak-d, lslidar, apriltag)
...
- Использует текущую ветку `main`

## 🏗️ Структура workflow

```mermaid
graph TD
    A[build-all.yml] -->|uses| B[build-base-images.yml]
    A -->|uses| C[build-main-services.yml]
    A -->|uses| D[build-vision-services.yml]
